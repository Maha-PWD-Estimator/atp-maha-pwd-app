...
var MP_MY_PHONE = ''; // cached saved mobile no. for current user (from users/{uid})
...
function openMarketplace(){
  ...
  mpDb().collection('users').doc(CU.uid).get().then(function(d){
    var savedPhone = (d.exists && d.data().phone) ? d.data().phone : '';
    MP_MY_PHONE = savedPhone || (CU.phoneNumber ? CU.phoneNumber.replace(/^\+91/, '') : '');
  }).catch(function(){ MP_MY_PHONE = CU.phoneNumber ? CU.phoneNumber.replace(/^\+91/, '') : ''; });
  mpShowRoleScreen();
}
...
function mpRenderPost(){
  ...
  body.innerHTML =
    '<div class="mpCard">'+
      '<b>तुमचा मोबाईल नं (Required)</b>'+
      '<input type="tel" id="mpPhone" maxlength="10" placeholder="10 अंकी मोबाईल नं" value="'+(MP_MY_PHONE||'')+'">'+
...
function mpSubmitPost(){
  ...
  var phone = (G('mpPhone')?G('mpPhone').value:'').replace(/[^0-9]/g,'');
  if(!scope){ showToast('Scope/description टाका','error'); return; }
  if(phone.length!==10){ showToast('वैध 10 अंकी मोबाईल नं टाका (Required)','error'); return; }
  var isBidding = !t.base;
  mpDb().collection('users').doc(CU.uid).set({phone:phone},{merge:true}).catch(function(){});
  MP_MY_PHONE = phone;
...
function mpStatusLabel(s){
  var map = {
    ...
    'cancel-requested':'⚠️ Cancellation — Fee भरायचं आहे',
    'cancel-fee-pending-verification':'⏳ Cancellation Fee Verify होत आहे',
    'cancelled':'❌ रद्द झालं'
  };
  return map[s]||s;
}
...
function mpRenderReqCard(id, r, viewerRole, container){
  ...
  if(viewerRole==='requestor'){
    if(r.status==='open' || r.status==='bidding-open' || r.status==='reassign-needed'){
      ...
      btnE.className='mpBtn gray'; btnE.style.marginLeft='.3rem'; btnE.textContent='Edit करा';
      btnE.onclick=function(){ mpEditRequest(id, r, detail); };
      detail.appendChild(btnE);

      var btnD = document.createElement('button');
      btnD.className='mpBtn red'; btnD.style.marginLeft='.3rem'; btnD.textContent='Delete करा';
      btnD.onclick=function(){ mpDeleteRequest(id); };
      detail.appendChild(btnD);
    }
    if(r.status==='cancel-requested'){
      detail.innerHTML =
        '<div style="background:#ffebee;color:#c62828;padding:.5rem;border-radius:6px;margin:.3rem 0">'+
        'Cancellation Fee: <b>₹'+r.cancelFeeAmount+'</b> (Assign झाल्यानंतर रद्द केल्यामुळे 10% fee लागू आहे)<br>'+
        'QR ने pay करा, नंतर screenshot खालील WhatsApp वर पाठवा:<br>'+
        '<a href="https://wa.me/'+MP_ADMIN_WHATSAPP+'?text='+encodeURIComponent('Cancellation fee screenshot for Request ID: '+id)+'" target="_blank">📱 Admin ला WhatsApp करा</a>'+
        '</div>'+
        '<button class="mpBtn" onclick="mpMarkCancelFeeSent(\''+id+'\')">मी Screenshot पाठवला ✅</button>';
    }
    if(r.status==='cancel-fee-pending-verification'){
      detail.innerHTML = '<div style="color:#888">Cancellation fee verify होत आहे — Admin लवकरच confirm करेल.</div>';
    }
    if(r.status==='cancelled'){
      detail.innerHTML = '<div style="color:#c62828">ही request रद्द करण्यात आली आहे.</div>';
    }
    ...
    if(r.status==='assigned' || r.status==='ready-for-payment' || r.status==='payment-pending-verification' || r.status==='payment-verified'){
      var btnC = document.createElement('button');
      btnC.className='mpBtn red'; btnC.style.marginLeft='.3rem'; btnC.textContent='काम रद्द करा (10% Fee लागेल)';
      btnC.onclick=function(){ mpRequestCancel(id, r); };
      detail.appendChild(btnC);
    }
  }
}

/* ---------- Edit / Delete (only before an estimator is assigned) ---------- */
function mpEditRequest(reqId, r, container){
  var opts = MP_TIERS.map(function(t){
    return '<option value="'+t.id+'"'+(t.id===r.tier?' selected':'')+'>'+t.label+(t.base?' — Pay ₹'+mpPayAmount(t.base):' — Bidding')+'</option>';
  }).join('');
  container.innerHTML =
    '<div class="mpCard">'+
      '<b>Estimate Value Tier</b>'+
      '<select id="mpEdTier_'+reqId+'">'+opts+'</select>'+
      '<b>कामाचे नाव / Scope</b>'+
      '<textarea id="mpEdScope_'+reqId+'" rows="3">'+(r.scope||'')+'</textarea>'+
      '<b>Deadline</b>'+
      '<input type="date" id="mpEdDeadline_'+reqId+'" value="'+(r.deadline||'')+'">'+
      '<div class="mpRow" style="margin-top:.4rem">'+
        '<button class="mpBtn" onclick="mpSaveEdit(\''+reqId+'\')">Save करा</button>'+
        '<button class="mpBtn gray" onclick="mpGoSub(\'myreq\')">रद्द करा</button>'+
      '</div>'+
    '</div>';
}
function mpSaveEdit(reqId){
  var tid = G('mpEdTier_'+reqId).value;
  var t = MP_TIERS.filter(function(x){return x.id===tid;})[0];
  var scope = G('mpEdScope_'+reqId).value.trim();
  var deadline = G('mpEdDeadline_'+reqId).value;
  if(!scope){ showToast('Scope टाका','error'); return; }
  var isBidding = !t.base;
  mpDb().collection('marketReqs').doc(reqId).update({
    tier: tid, tierLabel: t.label, type: isBidding?'bidding':'fixed',
    basePrice: t.base||null, payAmount: t.base?mpPayAmount(t.base):null,
    payoutAmount: t.base?mpPayoutAmount(t.base):null,
    scope: scope, deadline: deadline||''
  }).then(function(){ showToast('Update झालं ✅','success'); mpGoSub('myreq'); })
    .catch(function(e){ showToast('Error: '+e.message,'error'); });
}
function mpDeleteRequest(reqId){
  if(!confirm('ही request पूर्णपणे delete करायची? अजून कोणताही Estimator निवडलेला नसल्याने कोणतीही fee लागणार नाही.')) return;
  mpDb().collection('marketReqs').doc(reqId).delete()
    .then(function(){ showToast('Request delete झाली','success'); mpGoSub('myreq'); })
    .catch(function(e){ showToast('Error: '+e.message,'error'); });
}

/* ---------- Cancel after assignment (10% fee) ---------- */
function mpRequestCancel(reqId, r){
  var payBase = r.payAmount || (r.basePrice ? mpPayAmount(r.basePrice) : 0);
  var fee = Math.round(payBase*0.10);
  if(!confirm('हे काम रद्द करायचं? Estimator आधीच नेमला असल्याने रद्द करण्यासाठी 10% Cancellation Fee (₹'+fee+') भरावी लागेल. पुढे जायचं?')) return;
  mpDb().collection('marketReqs').doc(reqId).update({
    status:'cancel-requested', cancelFeeAmount: fee, cancelRequestedAt: mpFV().serverTimestamp()
  }).then(function(){ showToast('Cancellation सुरू केलं','info'); mpGoSub('myreq'); })
    .catch(function(e){ showToast('Error: '+e.message,'error'); });
}
function mpMarkCancelFeeSent(reqId){
  mpDb().collection('marketReqs').doc(reqId).update({
    status:'cancel-fee-pending-verification', cancelFeeSentAt: mpFV().serverTimestamp()
  }).then(function(){ showToast('Admin verify करेल','info'); mpGoSub('myreq'); });
}
  ...
  mpDb().collection('marketReqs')
    ...
    .then(function(snap){
      ...
      snap.forEach(function(d){
        ...
        docs.push({ id:d.id, r:r });
      });
      if(!docs.length){ body.innerHTML='<div style="color:#888">सध्या कोणतीही open job नाही.</div>'; return null; }
      // check which of these I've already applied to, so state persists across visits
      return Promise.all(docs.map(function(o){
        return mpDb().collection('marketReqs').doc(o.id).collection('applications').doc(CU.uid).get()
          .then(function(ad){ o.applied = ad.exists; o.appData = ad.exists ? ad.data() : null; return o; });
      }));
    })
    .then(function(docs){
      if(!docs) return;
      ...
      docs.forEach(function(o){
        ...
        if(o.applied){
          var bidNote = (r.type==='bidding' && o.appData && o.appData.bidAmount) ? (' — तुमचा दर ₹'+o.appData.bidAmount) : '';
          applyBox.innerHTML =
            '<div style="background:#e8f5e9;color:#18602f;padding:.4rem .6rem;border-radius:6px;font-weight:800;font-size:.72rem">✅ तुम्ही आधीच Apply केलं आहे'+bidNote+'</div>';
        }else{
          var phoneField = '<input type="tel" maxlength="10" placeholder="तुमचा मोबाईल नं (Required)" id="mpPhone_'+jobId+'" value="'+(MP_MY_PHONE||'')+'">';
          if(r.type==='bidding'){
            applyBox.innerHTML = phoneField+'<input type="number" placeholder="तुमचा दर ₹" id="mpBid_'+jobId+'"><textarea rows="2" placeholder="Message (optional)" id="mpMsg_'+jobId+'"></textarea>';
          }else{
            applyBox.innerHTML = phoneField+'<textarea rows="2" placeholder="Message (optional)" id="mpMsg_'+jobId+'"></textarea>';
          }
          var btn = document.createElement('button');
          btn.className='mpBtn'; btn.textContent='Apply करा';
          btn.onclick=function(){ mpApply(jobId, r); };
...
function mpApply(reqId, r){
  var phone = (G('mpPhone_'+reqId)?G('mpPhone_'+reqId).value:'').replace(/[^0-9]/g,'');
  if(phone.length!==10){ showToast('वैध 10 अंकी मोबाईल नं टाका (Required)','error'); return; }
  mpEnsureEstimatorProfile(CU.uid).then(function(prof){
    ...
    mpDb().collection('marketReqs').doc(reqId).collection('applications').doc(CU.uid).set(data)
      .then(function(){ showToast('Apply झालं ✅','success'); mpGoSub('jobs'); })
      .catch(function(e){ showToast('Error: '+e.message,'error'); });
  });
}
  ...
  mpDb().collection('marketReqs').where('selectedEstimator','==',CU.uid).orderBy('createdAt','desc').limit(30).get()
    .then(function(snap){
      ...
      snap.forEach(function(d){
        ...
        if(r.status==='cancel-requested' || r.status==='cancel-fee-pending-verification'){
          detail.innerHTML = '<div style="color:#c62828">⚠️ Requestor ने हे काम रद्द करण्याची विनंती केली आहे — Cancellation fee verify होत आहे.</div>';
        }
        if(r.status==='cancelled'){
          detail.innerHTML = '<div style="color:#c62828">❌ ही request रद्द करण्यात आली आहे.</div>';
        }
...
function mpRenderAdminSection(body){
  ...
  mpDb().collection('marketReqs').orderBy('createdAt','desc').limit(50).get().then(function(snap){
    ...
    snap.forEach(function(d){
      var r = d.data();
      docs.push({ id:d.id, r:r });
      if(r.postedBy) uidSet[r.postedBy]=true;
      if(r.selectedEstimator) uidSet[r.selectedEstimator]=true;
    });
    var uidList = Object.keys(uidSet);
    return Promise.all(uidList.map(function(u){
      return mpDb().collection('users').doc(u).get().then(function(ud){
        return { uid:u, data: ud.exists ? ud.data() : null };
      }).catch(function(){ return { uid:u, data:null }; });
    })).then(function(users){
      var map = {};
      users.forEach(function(uu){ map[uu.uid] = uu.data; });
      return { docs: docs, map: map };
    });
  }).then(function(res){
    var docs = res.docs, map = res.map;
    body.innerHTML='';
    docs.forEach(function(o){
      var reqId = o.id, r = o.r;
      var reqUser = map[r.postedBy];
      var estUser = r.selectedEstimator ? map[r.selectedEstimator] : null;
      var reqName = reqUser ? (reqUser.name || reqUser.phone || '') : '';
      var reqPhone = (reqUser && reqUser.phone) ? reqUser.phone : (r.requestorPhone || '');
      var card = document.createElement('div');
      card.className='mpCard';
      card.innerHTML =
        '<div class="mpRow" style="justify-content:space-between"><b>'+reqId.substring(0,8)+'</b><span class="mpBadge">'+mpStatusLabel(r.status)+'</span></div>'+
        '<div style="font-size:.7rem;color:#555">'+(r.tierLabel||'')+' · Pay ₹'+(r.payAmount||'-')+' · Payout ₹'+(r.payoutAmount||'-')+'</div>'+
        '<div style="font-size:.72rem;color:#333;margin-top:.25rem">👤 <b>Requestor:</b> '+esc(reqName||r.postedBy.substring(0,8))+(reqPhone?' ('+esc(reqPhone)+')':'')+'</div>'+
        (r.selectedEstimator ?
          ('<div style="font-size:.72rem;color:#333">🛠 <b>Estimator:</b> '+esc(estUser?(estUser.name||estUser.phone||''):'')+(estUser && estUser.name ? ' — ':'')+r.selectedEstimator.substring(0,8)+(estUser && estUser.phone ? ' ('+esc(estUser.phone)+')' : (r.selectedEstimatorPhone?(' ('+esc(r.selectedEstimatorPhone)+')'):''))+'</div>')
          : '');
      if(r.status==='payment-pending-verification'){
        ...
        b1.onclick=function(){ mpDb().collection('marketReqs').doc(reqId).update({status:'payment-verified',paymentVerifiedAt:mpFV().serverTimestamp()}).then(function(){showToast('Verified','success');mpRenderAdminSection();}); };
        card.appendChild(b1);
      }
      if(r.status==='requestor-satisfied'){
        ...
        b2.onclick=function(){ mpDb().collection('marketReqs').doc(reqId).update({status:'payout-done',payoutDoneAt:mpFV().serverTimestamp()}).then(function(){showToast('Payout marked','success');mpRenderAdminSection();}); };
        card.appendChild(b2);
      }
      if(r.status==='cancel-fee-pending-verification'){
        var bC=document.createElement('button'); bC.className='mpBtn red'; bC.textContent='Cancellation Fee Verify केलं ✅ (रद्द करा)';
        bC.onclick=function(){ mpDb().collection('marketReqs').doc(reqId).update({status:'cancelled',cancelledAt:mpFV().serverTimestamp()}).then(function(){showToast('Cancelled','success');mpRenderAdminSection();}); };
        card.appendChild(bC);
      }
      if(r.status==='assigned' || r.status==='ready-for-payment' || r.status==='payment-verified'){
        ...
        b3.onclick=function(){ mpReassign(reqId, r.selectedEstimator); };
        card.appendChild(b3);
      }
      body.appendChild(card);
    });
  }).catch(function(e){
    body.innerHTML='<div style="color:#c62828;padding:.6rem;font-size:.75rem">Error loading marketplace data: '+e.message+'<br><br>जर हे "Missing or insufficient permissions" म्हणत असेल, तर Firestore Security Rules अजून Firebase Console मध्ये update/publish केलेल्या नाहीत.</div>';
  });
}
