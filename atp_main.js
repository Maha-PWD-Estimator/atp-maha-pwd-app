...
if(typeof window.G!=='function'){window.G=function(id){return document.getElementById(id);};}
...
/* ── Optional 2nd Area Type (same list, second pick) ────────────────────
   Same Area-Type list (a to i) madhun 2 entries nivadta yaव्यात —
   dusरi selection dili tarach ticha % pahilya sobat add hoto (GR nusar
   don wegwegळe area conditions ek sathi lagu astat, jasa ki Municipal
   Council hadditit). Default: "— none —" nivadlela, mhanje kahich extra
   apply hot nahi jopर्यंत tumhi swतः dusरa area type nivadat nahi. */
function ensureAreaType2UI(){
  if(R('sAreaType2'))return;
  var areaSel=R('sAreaType');if(!areaSel||!areaSel.options||!areaSel.options.length)return;
  var host=areaSel.parentElement;if(!host||!host.parentNode)return;
  var wrap=document.createElement('div');
  wrap.id='sAreaType2Wrap';
  wrap.style.cssText='margin-top:.3rem;display:flex;align-items:center;gap:.35rem;font-size:.62rem;flex-wrap:wrap';
  var sel2=document.createElement('select');
  sel2.id='sAreaType2';
  sel2.setAttribute('onchange','updateAll()');
  sel2.style.cssText='font-size:.62rem;padding:.18rem .3rem;border:1px solid #ddd;border-radius:3px;max-width:230px';
  var optNone=document.createElement('option');
  optNone.value='0';optNone.setAttribute('data-label','');optNone.textContent='— None —';
  sel2.appendChild(optNone);
  for(var oi=0;oi<areaSel.options.length;oi++){
    var srcO=areaSel.options[oi];
    var o=document.createElement('option');
    o.value=srcO.value;
    var lbl=srcO.getAttribute('data-label');
    if(lbl!=null)o.setAttribute('data-label',lbl);
    o.textContent=srcO.textContent;
    sel2.appendChild(o);
  }
  var lblSpan=document.createElement('span');
  lblSpan.style.cssText='color:var(--mu)';
  lblSpan.textContent='+ 2nd Area Type (garaj asel tar):';
...
function gS(){
  ...
  var areaSel2=R('sAreaType2');
  var areaPct2=areaSel2?(parseFloat(areaSel2.value)||0):0;
  var areaLbl2=areaSel2&&areaSel2.options[areaSel2.selectedIndex]?
    (areaSel2.options[areaSel2.selectedIndex].getAttribute('data-label')||''):'' ;
  return{gst:pf('sGST')||18,cont:pfD('sCont',2),li:pfD('sLI',1),
    ...
    areaPct2:areaPct2,areaLbl2:areaLbl2,
...
var CF_MATS=['rubble','overmetal','sand','scr_sand','cement_bags','m40mm','m20mm','m12_10','quarry','bricks','ms_bars','bitumen','tiles_hr','flooring','gi_sheet','concrete_block'];
var CF_MAT_LABELS=['Rubble','Overmetal','Sand','Scr.Sand','Cement','M40mm','M20mm','M12mm','Quarry','Bricks','Steel','Bitumen','Tiles HR','Flooring','GI Sheet','Concrete Block'];
/* Units aligned index-for-index with CF_MATS - single source of truth so every CF-consuming
   function (item CF editor, consumption chart, etc.) stays in sync when a new column is added */
var CF_MAT_UNITS=['M3','M3','M3','M3','Bags','M3','M3','M3','M3','Nos','MT','MT','M3','M2','MT','M3'];
...
function rCFTab(){
  ...
  tb.innerHTML=h||'<tr><td colspan="16" style="text-align:center;padding:1.5rem;color:#aaa">No items found</td></tr>';
...
function saveCFtoCloud(){
  ...
  firebase.firestore().collection("config").doc("cfmap").set({
    cf:cfData,updatedAt:firebase.firestore.FieldValue.serverTimestamp(),updatedBy:(CU&&CU.email)||'unknown'
  }).then(function(){
...
function saveCFandDownload(){
  ...
  for(var i=0;i<keys.length;i++){
    ...
    if(mats.length)entryLines.push('  "'+no+'":{'+mats.join(',')+'}');
  }
  var newCFMAP='var CF_MAP={\n'+entryLines.join(',\n')+'\n};';

  /* Get current HTML */
  var html=document.documentElement.outerHTML;

  /* CF_MAP now loads from a separate file via <script src="cf_data.js"></script> — it is
     NOT inline in the HTML, so we can't string-replace "var CF_MAP=" inside html directly.
     Instead: swap that <script src="cf_data.js"> tag for an inline <script> carrying the
     freshly-saved CF_MAP, so the downloaded HTML is a fully standalone file (no separate
     cf_data.js needed) that the user can edit further and re-download anytime. */
  var srcTagRe=/<script\s+src=["']cf_data\.js["']\s*>\s*<\/script>/i;
  if(srcTagRe.test(html)){
    html=html.replace(srcTagRe,'<script>\n'+newCFMAP+'\n</script>');
  } else {
    /* Fallback for older files where CF_MAP was already inline */
    var cfStart=html.indexOf('var CF_MAP=');
    var cfEnd=html.indexOf('};',cfStart)+2;
    if(cfStart<0||cfEnd<2){alert('CF_MAP cf_data.js script tag ki inline — konthehi sapadla nahi. Kripaya file check kara.');return;}
    html=html.substring(0,cfStart)+newCFMAP+html.substring(cfEnd);
  }
  ...
  var a=document.createElement('a');
  a.href=url;
  a.download='Maha_PWD_Estimator_CF_saved.html';
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
  alert('✅ CF values HTML madhe embed karun save zalya!\nHi download zaleli standalone HTML file cf_data.js shivay pan swataहच chalel — CF values tyat baked-in astil.');
}
...
function recalcItem(it){
  ...
  var _aS=typeof gS==='function'?gS():{areaPct:0,areaLbl:'',areaPct2:0,areaLbl2:''};
  if(_aS.areaPct>0){
  ...
  if(_aS.areaPct2>0){
    var _aAdd2=Math.round(it.baseRate*_aS.areaPct2/100*100)/100;
    lm.push({mat:'area_pct2',label:'+'+_aS.areaPct2+'% Specific Area ('+_aS.areaLbl2+')',
      factor:_aS.areaPct2,conv:1,km:0,chg:0,add:_aAdd2});
    fr=Math.round((fr+_aAdd2)*100)/100;
  }
...
function selItem(no){
  ...
  /* Direct Measurement mode — skip the search-tab form, go straight to
     the Measurement Sheet and let the user type L/B/D right there. */
  if(window._atpDirectMeas){atpAddSelDirect();return;}
...
function msrUpd(iIdx,rIdx,el,field){
  ...
  if(field==="lbl"){r.lbl=el.value;return;}
    ...
    if(field==="pct")r.pct=parseFloat(el.value)||0;
    else if(field==="pctSrc")r.pctSrc=(el.value==="self")?"self":parseInt(el.value,10);
  }
  ...
  /* Recalc — this item, every linked row, every % row, then Abstract */
  msrSyncAllRows();
  for(var _ai=0;_ai<items.length;_ai++)items[_ai]=recalcItem(items[_ai]);
  msrRefreshAllDisplays();
...
function delMeasRow(itemIdx,rowIdx){
  ...
  /* Recalc total (includes % rows) */
  msrRecalcItem(itemIdx);
  if(it.rows.length===0){
...
function openCFEditor(itemIdx){
  ...
  /* All available materials - built from CF_MATS so it always matches the CF Table tab */
  var mats=CF_MATS.map(function(k,idx){
    return {k:k,lbl:CF_MAT_LABELS[idx],u:CF_MAT_UNITS[idx]||''};
  });
...
function msrAddRow(iIdx){
  ...
  var totIdx=it.rows.findIndex(function(r){return r.isTot||r.isPct;});
...
function msrAddDeduction(iIdx){
  ...
  document.getElementById('_dAdd').onclick=function(){
    ...
    var _pi=it.rows.findIndex(function(r){return r.isTot||r.isPct;});
    if(_pi>=0)it.rows.splice(_pi,0,row);else it.rows.push(row);
    msrRecalcItem(iIdx);
    items[iIdx]=recalcItem(items[iIdx]);
    rMS();updateAll();
...
/* Which fields get a LIVE link on paste. Default: all fields selected (N,L,B,D). */
window._msrLinkFields={n:true,l:true,b:true,d:true,qty:false};
window._msrLinkPct=100;
...
function msrActivateCopy(){
  ...
  window._msrLinkPct=100;
  rMS();
  if(typeof showToast==='function') showToast('Copy Mode ON — check rows to copy, choose fields to link, then click Paste Linked.','info');
}

function msrToggleLinkField(f,el){
  window._msrLinkFields[f]=!!el.checked;
  /* Qty link is a whole-row link — it supersedes the individual fields */
  if(f==='qty'&&el.checked){window._msrLinkFields.n=window._msrLinkFields.l=window._msrLinkFields.b=window._msrLinkFields.d=false;}
  rMS();
}
function msrSetLinkPct(el){window._msrLinkPct=parseFloat(el.value);if(isNaN(window._msrLinkPct))window._msrLinkPct=100;}
...
/* Backward compatible: old saved rows had _linkedFrom without .fields → treat as full link (n,l,b,d) */
function msrLinkFieldsOf(r){
  if(!r||!r._linkedFrom)return[];
  return r._linkedFrom.fields||['n','l','b','d'];
}
function msrHasQtyLinkRow(r){return msrLinkFieldsOf(r).indexOf('qty')>-1;}
...
/* ── LEGACY: item-level "Qty % Link" kadhun taakla ─────────────
   Tyachi jaga aata "% Row" ne ghetli aahe (row-level, item chya total madhe
   jate). Jhune saved estimates madhe it.qtyLink asel tar to safaine kadhto,
   jene karun total nehami rows varunach yeil. */
function msrStripLegacyQtyLinks(){
  if(typeof items==='undefined'||!items)return;
  for(var k=0;k<items.length;k++){
    if(items[k]&&items[k].qtyLink)delete items[k].qtyLink;
  }
}

/* ══════════════════════════════════════════════════════════════════════
   MEASUREMENT ENGINE  (% rows + full field linking + central recalc)
   ────────────────────────────────────────────────────────────────────
   Row types inside it.rows:
     normal    {lbl,n,l,b,d,fl,qty}
     deduction {... isDeduct:true}          -> qty negative
     % row     {isPct:true, pct:5, pctSrc:'self'|<itemIdx>, lbl, qty}
     total     {isTot:true}
   A % row ADDS its qty into the item total, so it reaches BOQ/Abstract
   exactly like any other row.
   ══════════════════════════════════════════════════════════════════ */

/* Base quantity a % row is computed on */
function msrPctBase(iIdx,r){
  var src=(r&&r.pctSrc!=null)?r.pctSrc:'self';
  if(src==='self'||src===''||src==null){
    var it=items[iIdx];if(!it||!it.rows)return 0;
    var b=0;
    it.rows.forEach(function(rw){if(rw.isTot||rw.isPct)return;b+=rw.qty||0;});
    return Math.round(b*1000)/1000;
  }
  var si=parseInt(src,10);
  if(isNaN(si)||si<0||si>=items.length||si===iIdx||!items[si])return 0;
  return items[si].qty||0;
}

/* Quantity of ONE row (handles normal / deduction / qty-linked / % ) */
function msrCalcRowQty(iIdx,r){
  if(!r||r.isTot)return (r&&r.qty)||0;
  if(r.isPct){
    r.qty=Math.round(msrPctBase(iIdx,r)*(parseFloat(r.pct)||0)/100*1000)/1000;
    return r.qty;
  }
  if(r._linkedFrom&&msrLinkFieldsOf(r).indexOf('qty')>-1){
    var sIt=items[r._linkedFrom.iIdx];
    var s=(sIt&&sIt.rows)?sIt.rows[r._linkedFrom.rIdx]:null;
    if(s){
      var lp=(r._linkedFrom.pct!=null?r._linkedFrom.pct:100);
      r.qty=Math.round((s.qty||0)*lp/100*1000)/1000;
      return r.qty;
    }
  }
  var t=(r.n||1);
...
/* Recompute every row of ONE item (normal rows first, then % rows) */
function msrRecalcItem(iIdx){
  var it=items[iIdx];if(!it||!it.rows)return 0;
  it.rows.forEach(function(r){if(!r.isTot&&!r.isPct)msrCalcRowQty(iIdx,r);});
  it.rows.forEach(function(r){if(r.isPct)msrCalcRowQty(iIdx,r);});
  var tot=0;it.rows.forEach(function(r){if(!r.isTot)tot+=r.qty||0;});
  it.qty=Math.round(tot*1000)/1000;
  return it.qty;
}

/* Mirror linked fields from their source row + resolve stored formulas,
   then recompute qty for every item. Several passes so chains settle. */
function msrSyncAllRows(){
  for(var p=0;p<3;p++){
    for(var ii=0;ii<items.length;ii++){
      (function(ii){
        var it=items[ii];if(!it||!it.rows)return;
        it.rows.forEach(function(r){
          if(r.isTot||r.isPct)return;
          if(r._linkedFrom){
            var sIt=items[r._linkedFrom.iIdx];
            var sr=(sIt&&sIt.rows)?sIt.rows[r._linkedFrom.rIdx]:null;
            if(sr){
              var fl=msrLinkFieldsOf(r);
              var allq=fl.indexOf('qty')>-1;
              if(allq||fl.indexOf('n')>-1)r.n=sr.n||1;
              if(allq||fl.indexOf('l')>-1)r.l=(sr.l!=null?sr.l:0);
              if(allq||fl.indexOf('b')>-1)r.b=(sr.b!=null?sr.b:0);
              if(allq||fl.indexOf('d')>-1)r.d=(sr.d!=null?sr.d:0);
              if(fl.length)r.isDeduct=(sr.isDeduct||false);
            }
          } else if(r._f){
            if(r._f.l){var v1=msrResolve(r._f.l,ii);if(v1!==null)r.l=v1;}
            if(r._f.b){var v2=msrResolve(r._f.b,ii);if(v2!==null)r.b=v2;}
            if(r._f.d){var v3=msrResolve(r._f.d,ii);if(v3!==null)r.d=v3;}
          }
        });
        msrRecalcItem(ii);
      })(ii);
...
/* Refresh qty cells + total line of ONE item (no re-render, keeps focus) */
function msrRefreshItemDisplay(iIdx){
  var it=items[iIdx];if(!it||!it.rows)return;
  it.rows.forEach(function(r,ri){
    if(r.isTot)return;
    var qel=R('msrq_'+iIdx+'_'+ri);
    if(qel){
      var q=r.qty||0,qa=Math.abs(q);
      qel.innerHTML=(q<0)
        ?'<span style="color:#c62828;font-weight:700">(-'+qa.toFixed(3)+')</span>'
        :'<span style="font-weight:700'+(r.isPct?';color:#6a1b9a':'')+'">'+q.toFixed(3)+'</span>';
    }
    if(r.isPct){
      var bel=R('msrpb_'+iIdx+'_'+ri);
      if(bel)bel.textContent=msrPctBase(iIdx,r).toFixed(3);
      var bel2=R('msrpb2_'+iIdx+'_'+ri);
      if(bel2)bel2.textContent=msrPctBase(iIdx,r).toFixed(3);
    }
  });
  var totEl=R('mstot_'+iIdx);if(totEl)totEl.textContent=(it.qty||0).toFixed(3);
  var lblEl=R('mstotlbl_'+iIdx);
  if(lblEl)lblEl.innerHTML=msrTotalLabel(it);
}

/* The "Total = a − b + c%" caption under an item */
function msrTotalLabel(it){
  var ps=0,ds=0,pc=0;
  (it.rows||[]).forEach(function(rw){
    if(rw.isTot)return;
    if(rw.isPct){pc+=rw.qty||0;return;}
    if(rw.isDeduct)ds+=Math.abs(rw.qty||0);else ps+=rw.qty||0;
  });
  ps=Math.round(ps*1000)/1000;ds=Math.round(ds*1000)/1000;pc=Math.round(pc*1000)/1000;
  if(ds<=0&&Math.abs(pc)<0.0005)return 'Total';
  var t='Total = <b>'+ps.toFixed(3)+'</b>';
  if(ds>0)t+=' &minus; <span style="color:#c62828"><b>'+ds.toFixed(3)+'</b></span>';
  if(Math.abs(pc)>=0.0005)t+=' '+(pc<0?'&minus;':'+')+' <span style="color:#6a1b9a"><b>'+Math.abs(pc).toFixed(3)+'</b> (%)</span>';
  t+=' = <span style="color:#1a237e"><b>'+(it.qty||0).toFixed(3)+'</b></span>';
  return t;
}

/* Push fresh values into every visible input / qty cell without re-render */
function msrRefreshAllDisplays(){
  items.forEach(function(it,ii){
    if(!it.rows)return;
    it.rows.forEach(function(r,ri){
      if(r.isTot||!r._linkedFrom)return;
      var row=R('msr_'+ii+'_'+ri);if(!row)return;
      var fl=msrLinkFieldsOf(r),allq=fl.indexOf('qty')>-1;
      row.querySelectorAll('input').forEach(function(inp){
        var oi=inp.getAttribute('oninput')||'';
        if((allq||fl.indexOf('n')>-1)&&oi.indexOf(",'n')")>-1)inp.value=r.n||1;
        if((allq||fl.indexOf('l')>-1)&&oi.indexOf(",'l')")>-1)inp.value=r.l||'';
        if((allq||fl.indexOf('b')>-1)&&oi.indexOf(",'b')")>-1)inp.value=r.b||'';
        if((allq||fl.indexOf('d')>-1)&&oi.indexOf(",'d')")>-1)inp.value=r.d||'';
      });
    });
    msrRefreshItemDisplay(ii);
  });
}

/* ── + % Row ─────────────────────────────────────────────────────── */
function msrAddPctRow(iIdx){
  var it=items[iIdx];if(!it)return;
  if(!it.rows)it.rows=[];
  var opts='<option value="self">या item च्या rows ची Total</option>';
  items.forEach(function(oit,oi){
    if(oi===iIdx)return;
    opts+='<option value="'+oi+'">'+_escER((oi+1)+') '+oit.no+' - '+(oit.desc||'').substring(0,40))+'</option>';
  });
  var modal=document.createElement('div');
  modal.style.cssText='position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.5);z-index:9999;display:flex;align-items:center;justify-content:center';
  modal.innerHTML='<div style="background:#fff;border-radius:10px;padding:1.2rem 1.5rem;width:380px;box-shadow:0 6px 24px rgba(0,0,0,.3)">'
    +'<b style="color:#6a1b9a;font-size:.88rem">% Row \u091f\u093e\u0915\u093e</b>'
    +'<div style="font-size:.64rem;color:#888;margin:.15rem 0 .6rem">Qty = % \u00d7 base. \u0939\u0940 qty item \u091a\u094d\u092f\u093e total \u092e\u0927\u094d\u092f\u0947 <b>add</b> \u0939\u094b\u0908\u0932 (Abstract \u092a\u0930\u094d\u092f\u0902\u0924 \u091c\u093e\u0908\u0932).</div>'
    +'<div style="font-size:.65rem;color:#555;margin-bottom:.18rem">Description</div>'
    +'<input id="_pLbl" type="text" placeholder="e.g. Add 5% for laps / wastage" style="width:100%;padding:.3rem;border:1.5px solid #6a1b9a;border-radius:5px;font-size:.72rem;margin-bottom:.5rem;box-sizing:border-box">'
    +'<div style="font-size:.65rem;color:#555;margin-bottom:.18rem">Base (\u0915\u0936\u093e\u091a\u0947 %)</div>'
    +'<select id="_pSrc" style="width:100%;padding:.3rem;border:1px solid #ccc;border-radius:5px;font-size:.7rem;margin-bottom:.5rem;box-sizing:border-box">'+opts+'</select>'
    +'<div style="font-size:.65rem;color:#555;margin-bottom:.18rem">% (minus \u0938\u093e\u0920\u0940 \u0909\u0923\u0947 \u0935\u093e\u092a\u0930\u093e, e.g. -5)</div>'
    +'<input id="_pPct" type="number" step="any" value="5" style="width:100%;padding:.3rem;border:1px solid #ccc;border-radius:5px;font-size:.72rem;margin-bottom:.4rem;box-sizing:border-box">'
    +'<div id="_pPrev" style="font-size:.68rem;color:#6a1b9a;font-weight:700;text-align:right;margin-bottom:.5rem;min-height:1rem"></div>'
    +'<div style="display:flex;gap:.4rem;justify-content:flex-end">'
    +'<button id="_pCancel" style="padding:.28rem .8rem;border:1px solid #ccc;border-radius:5px;font-size:.72rem;cursor:pointer">Cancel</button>'
    +'<button id="_pAdd" style="padding:.28rem .8rem;border:none;border-radius:5px;font-size:.72rem;cursor:pointer;background:#6a1b9a;color:#fff;font-weight:700">+ % Row</button>'
    +'</div></div>';
  document.body.appendChild(modal);
  function prev(){
    var src=document.getElementById('_pSrc').value;
    var pct=parseFloat(document.getElementById('_pPct').value)||0;
    var base=msrPctBase(iIdx,{pctSrc:(src==='self'?'self':parseInt(src,10))});
    var el=document.getElementById('_pPrev');
    if(el)el.textContent='Base '+base.toFixed(3)+' \u00d7 '+pct+'% = '+(Math.round(base*pct/100*1000)/1000).toFixed(3)+' '+(it.unit||'');
  }
  ['_pSrc','_pPct'].forEach(function(id){var el=document.getElementById(id);if(el)el.addEventListener('input',prev);});
  prev();
  setTimeout(function(){var el=document.getElementById('_pLbl');if(el)el.focus();},80);
  document.getElementById('_pCancel').onclick=function(){modal.remove();};
  document.getElementById('_pAdd').onclick=function(){
    var pct=parseFloat(document.getElementById('_pPct').value);
    if(isNaN(pct)||pct===0){alert('% value takaa');return;}
    var srcV=document.getElementById('_pSrc').value;
    var row={isPct:true,
      pct:Math.round(pct*100)/100,
      pctSrc:(srcV==='self'?'self':parseInt(srcV,10)),
      lbl:(document.getElementById('_pLbl').value.trim())||('Add '+pct+'%'),
      n:1,l:0,b:0,d:0,fl:0,qty:0};
    modal.remove();
    var ti=it.rows.findIndex(function(r){return r.isTot;});
    if(ti>=0)it.rows.splice(ti,0,row);else it.rows.push(row);
    msrRecalcItem(iIdx);
    items[iIdx]=recalcItem(items[iIdx]);
    rMS();updateAll();
    if(typeof showToast==='function')showToast('% Row add zala — total ani Abstract madhe reflect hoil','success');
  };
}

/* ── DIRECT MEASUREMENT MODE ──────────────────────────────────────── */
window._atpDirectMeas=(function(){try{var v=localStorage.getItem('atp_direct_meas');return v===null?true:v==='1';}catch(e){return true;}})();
function atpToggleDirectMeas(el){
  window._atpDirectMeas=!!el.checked;
  try{localStorage.setItem('atp_direct_meas',el.checked?'1':'0');}catch(e){}
  if(typeof showToast==='function')showToast(el.checked
    ?'Direct Measurement ON — item select kelyavar thet Measurement Sheet var jail'
    :'Direct Measurement OFF — juna Add form ughadel','info');
}
/* Add the selected SSR item straight into the estimate with one blank row,
   jump to the Measurement Sheet tab and focus that row. */
function atpAddSelDirect(){
  if(!selSSR)return;
  var cf=selSSR.cf||{},sv=selSSR.scadaVal||0;
  var res=calcItemLead(cf);
  var baseRate=selSSR.rate+res.total+sv;
  var fr=Math.round(baseRate*100)/100;
  var blank={lbl:'',n:1,l:0,b:0,d:0,qty:0,fl:0,flRate:fr};
  var existIdx=-1,k;
  for(k=0;k<items.length;k++){if(items[k].no===selSSR.no){existIdx=k;break;}}
  var tIdx,rIdx;
  if(existIdx>=0){
    var ex=items[existIdx];
    if(!ex.rows)ex.rows=[];
    var ti=ex.rows.findIndex(function(r){return r.isTot||r.isPct;});
    if(ti>=0){ex.rows.splice(ti,0,blank);rIdx=ti;}
    else{ex.rows.push(blank);rIdx=ex.rows.length-1;}
    tIdx=existIdx;
  } else {
    items.push({id:Date.now(),no:selSSR.no,desc:selSSR.desc,unit:selSSR.unit,spec:selSSR.spec||'',
      baseRate:selSSR.rate,cf:cf,scadaVal:sv,
      leadPerUnit:Math.round(res.total*100)/100,leadMats:res.mats,
      finalRate:fr,qty:0,rows:[blank],amount:0});
    tIdx=items.length-1;rIdx=0;
  }
  closeForm();
  updateAll();
  if(typeof goTab==='function')goTab(5);
  setTimeout(function(){
    var row=R('msr_'+tIdx+'_'+rIdx);
    if(!row)return;
    row.scrollIntoView({behavior:'smooth',block:'center'});
    var ob=row.style.background;
    row.style.background='#fff3cd';
    setTimeout(function(){row.style.background=ob;},1500);
    var inps=row.querySelectorAll('input');
    if(inps&&inps.length)inps[0].focus();
  },260);
  if(typeof showToast==='function')showToast('Item add zala — ithech L / B / D taka','success');
}
function _atpInjectDirectToggle(){
  if(R('_dmToggleWrap'))return;
  var info=R('srchInfo');
  var host=info?info.parentNode:null;
  if(!host){var sb=R('srchIn');host=sb?sb.parentNode:null;}
  if(!host)return;
  var w=document.createElement('label');
  w.id='_dmToggleWrap';
  w.style.cssText='display:inline-flex;align-items:center;gap:.25rem;font-size:.6rem;color:#6a1b9a;margin-left:.5rem;cursor:pointer;user-select:none;white-space:nowrap';
  w.innerHTML='<input type="checkbox" id="_dmToggle" '+(window._atpDirectMeas?'checked':'')
    +' style="width:13px;height:13px;accent-color:#6a1b9a" onchange="atpToggleDirectMeas(this)">Direct Measurement Sheet';
  host.appendChild(w);
}
if(document.readyState!=='loading')setTimeout(_atpInjectDirectToggle,400);
else document.addEventListener('DOMContentLoaded',function(){setTimeout(_atpInjectDirectToggle,400);});
...
function msrPasteRows(destIIdx){
  ...
  /* Which fields to keep LIVE-linked — user's choice via checkboxes (default L+B) */
  var _lf=window._msrLinkFields||{n:false,l:true,b:true,d:false,qty:false};
  var _linkedFieldList=['n','l','b','d','qty'].filter(function(f){return _lf[f];});
  if(_lf.qty)_linkedFieldList=['qty'];  /* whole-row Qty link */
  if(!_linkedFieldList.length){alert('Kripaya kiman ek field (N/L/B/D/Qty) link sathi nivda.');return;}
  var _linkPct=parseFloat(window._msrLinkPct);if(isNaN(_linkPct)||_linkPct<=0)_linkPct=100;
  ...
  selList.forEach(function(src){
    ...
    /* Fields NOT chosen for linking are copied once but stay independent/editable */
    var newRow={
      lbl:      srcR.lbl||'',
      n:        srcR.n||1,
      l:        srcR.l||0,
      ...
      _linkedFrom: {iIdx: src.iIdx, rIdx: src.rIdx, fields: _linkedFieldList.slice(), pct: _linkPct}  /* index ref — survives JSON save/load */
    };
    if(_lf.qty){
      newRow.qty=Math.round((srcR.qty||0)*_linkPct/100*1000)/1000;
    } else {
      var _t=(newRow.n||1);
    ...
    /* Insert before any isTot / % row so % rows stay at the bottom */
    var _ti=destIt.rows.findIndex(function(r){return r.isTot||r.isPct;});
    if(_ti>=0) destIt.rows.splice(_ti,0,newRow);
    else destIt.rows.push(newRow);
  });

  msrRecalcItem(destIIdx);
  ...
  rMS();
  updateAll();
  if(typeof showToast==='function') showToast(selList.length+' row(s) pasted — '+_linkedFieldList.map(function(f){return f.toUpperCase();}).join('+')+' linked live, rest independent.','success');
}
...
function updateAll(){
  getBitChange();
  /* rows -> item qty (normal + linked + % rows) BEFORE BOQ/Abstract render */
  msrStripLegacyQtyLinks();
  msrSyncAllRows();
...
function rRA(){
  ...
  for(i=0;i<items.length;i++){
    ...
    for(j=0;j<(it.leadMats||[]).length;j++){
      ...
      } else if(m.mat==='area_pct'||m.mat==='area_pct2'){
        noteTxt='on base rate only';
      } else {
        noteTxt=m.factor+(m.conv!==1?'\u00d7'+m.conv:'')+' \u00d7 '+m.km+'km';
      }
      subRows.push({lbl:((m.mat==='area_pct'||m.mat==='area_pct2')?m.label:'+ Lead: '+m.label),
...
function rMS(){
  ...
  for(i=0;i<items.length;i++){
    ...
    for(ri=0;ri<it.rows.length;ri++){
      ...
      if(!r.isTot){
        ...
        if(r._linkedFrom){
          ...
          if(_s){
            var _flist=msrLinkFieldsOf(r);
            var _allq=_flist.indexOf('qty')>-1;
            if(_allq||_flist.indexOf('l')>-1) r.l=(_s.l!=null?_s.l:0);
            if(_allq||_flist.indexOf('b')>-1) r.b=(_s.b!=null?_s.b:0);
            if(_allq||_flist.indexOf('d')>-1) r.d=(_s.d!=null?_s.d:0);
            if(_allq||_flist.indexOf('n')>-1) r.n=(_s.n||1);
            if(_flist.length) r.isDeduct=(_s.isDeduct||false); /* mirror deduction status */
          }
        } else if(r._f){
        ...
        /* central helper — handles normal / deduction / Qty-linked / % */
        msrCalcRowQty(i,r);
      }
      ...
      if(r.isPct){
        var _pOpts='<option value="self"'+((r.pctSrc==null||r.pctSrc==='self')?' selected':'')+'>own rows</option>';
        items.forEach(function(oit,oi){
          if(oi===i)return;
          _pOpts+='<option value="'+oi+'"'+(String(r.pctSrc)===String(oi)?' selected':'')+'>'+_escER('Item '+(oi+1)+' ('+oit.no+')')+'</option>';
        });
        var _pBase=msrPctBase(i,r);
        h+='<tr id="msr_'+i+'_'+ri+'" style="background:#f7f0fb">'
          +'<td></td><td></td><td></td>'
          +'<td style="text-align:center"><span style="display:inline-block;font-size:.46rem;font-weight:700;background:#6a1b9a;color:#fff;padding:.1rem .3rem;border-radius:3px;letter-spacing:.03em">%</span></td>'
          +'<td><input style="'+IS+';width:100%;color:#4a148c" value="'+esc(r.lbl||'')+'" placeholder="e.g. Add 5% for laps" oninput="msrUpd('+i+','+ri+',this,\'lbl\')">'
            +'<span class="noprt" style="font-size:.5rem;color:#6a1b9a;margin-left:.15rem;white-space:nowrap">of <select onchange="msrUpd('+i+','+ri+',this,\'pctSrc\')" style="font-size:.5rem;padding:0;border:1px solid #ce93d8;border-radius:2px">'+_pOpts+'</select></span></td>'
          +'<td class="r"><input style="'+IS+';width:44px;text-align:right;color:#4a148c;font-weight:700" type="number" step="any" value="'+(r.pct!=null?r.pct:0)+'" oninput="msrUpd('+i+','+ri+',this,\'pct\')" title="Percentage"></td>'
          +'<td class="r" style="font-size:.58rem;color:#6a1b9a;font-weight:700">%</td>'
          +'<td class="r" style="font-size:.56rem;color:var(--mu)" id="msrpb_'+i+'_'+ri+'">'+_pBase.toFixed(3)+'</td>'
          +'<td></td>'
          +'<td class="r" id="msrq_'+i+'_'+ri+'"><span style="font-weight:700;color:#6a1b9a">'+(r.qty||0).toFixed(3)+'</span></td>'
          +'<td style="font-size:.56rem">'+(multiRow?'':it.unit)+'</td>'
          +'<td class="noprt" style="text-align:center"><button onclick="delMeasRow('+i+','+ri+')" class="btn-sm" style="background:#c62828;color:#fff;font-size:.52rem;padding:.1rem .25rem">✕</button></td>'
          +'</tr>';
        if(r.comment){
          h+='<tr style="background:#f7f0fb"><td></td><td></td><td></td><td></td>'
            +'<td colspan="8" style="font-size:.56rem;color:#6a1b9a;padding:.1rem .3rem;font-style:italic">'+esc(r.comment)+'</td></tr>';
        }
      } else if(isTotRow){
        ...
        /* Pre-compute link info — link can now be field-specific (only N/L/B/D chosen at paste time) */
        var _isLinked=!!r._linkedFrom;
        var _lnkFields=msrLinkFieldsOf(r);
        var _qLinked=_lnkFields.indexOf('qty')>-1;
        var _nLinked=_qLinked||_lnkFields.indexOf('n')>-1;
        var _lLinked=_qLinked||_lnkFields.indexOf('l')>-1;
        var _bLinked=_qLinked||_lnkFields.indexOf('b')>-1;
        var _dLinked=_qLinked||_lnkFields.indexOf('d')>-1;
        var _lnkTitle='';
        if(_isLinked){
          ...
          var _fNames={n:'N',l:'L',b:'B',d:'D',qty:'Qty'};
          var _fTxt=_lnkFields.map(function(f){return _fNames[f];}).join('+')||'—';
          var _lpTxt=(_lf2.pct!=null&&_lf2.pct!==100)?(' × '+_lf2.pct+'%'):'';
          _lnkTitle='🔗 '+_fTxt+_lpTxt+' linked from Item '+(_lf2.iIdx+1)+', Row '+(_lf2.rIdx+1)+' ('+_srcDescLnk2+') — edit source row to update';
        }
        ...
        h+='<td><input style="'+IS+';width:100%;'+(r._f&&r._f.lbl?'background:#fffde7':'')
          ...
          +(_isLinked?'<span class="ms-link-badge noprt" title="'+_lnkTitle+'" style="font-size:.45rem;background:#1565c0;color:#fff;padding:.05rem .25rem;border-radius:2px;margin-left:.15rem;cursor:help;white-space:nowrap">🔗 '+_lnkFields.map(function(f){return f.toUpperCase();}).join('+')+' Linked</span>':'')
        ...
        h+='<td class="r"><input style="'+IS+';width:38px;text-align:right'
          +(_nLinked?';background:#e3f2fd;cursor:help;pointer-events:none':(_ded?';background:#fff0f0':''))
          +'" '+IF+(_nLinked?' readonly':'')+' type="number" step="any" min="0" value="'+(r.n||1)
          +'" oninput="msrUpd('+i+','+ri+',this,\'n\')" title="'+(_nLinked?_lnkTitle:'')+'"></td>';
        ...
        h+='<td class="r"><input style="'+IS+';width:55px;text-align:right;font-family:monospace'
          +(_lLinked?';background:#e3f2fd;cursor:help':(_ded?';background:#fff0f0':''))
          +';" '+IF+' value="'+esc(String(_lDisp===0?'':_lDisp))
          +'" placeholder="—" '+(_lLinked?'readonly':'')+' oninput="msrUpd('+i+','+ri+',this,\'l\')" title="'+(_lLinked?_lnkTitle:'Type value or avg(a,b,c) for average')+'">'+_lComment+'</td>';
        ...
        h+='<td class="r"><input style="'+IS+';width:55px;text-align:right;font-family:monospace'
          +(_bLinked?';background:#e3f2fd;cursor:help':(_ded?';background:#fff0f0':''))
          +';" '+IF+' value="'+esc(String(_bDisp===0?'':_bDisp))
          +'" placeholder="—" '+(_bLinked?'readonly':'')+' oninput="msrUpd('+i+','+ri+',this,\'b\')" title="'+(_bLinked?_lnkTitle:'Type value or avg(a,b,c) for average')+'">'+_bComment+'</td>';
        ...
        h+='<td class="r"><input style="'+IS+';width:55px;text-align:right;font-family:monospace'
          +(_dLinked?';background:#e3f2fd;cursor:help':(_ded?';background:#fff0f0':''))
          +';" '+IF+' value="'+esc(String(_dDisp===0?'':_dDisp))
          +'" placeholder="—" '+(_dLinked?'readonly':'')+' oninput="msrUpd('+i+','+ri+',this,\'d\')" title="'+(_dLinked?_lnkTitle:'Type value or avg(a,b,c) for average')+'">'+_dComment+'</td>';
        /* Qty display */
    ...
    h+='<tr class="noprt"><td colspan="11" style="padding:.25rem .4rem;border-top:1px solid #e8e8e8">'
      +(window._msrSelActive?('<span style="font-size:.56rem;color:var(--mu);margin-right:.3rem">Link:</span>'
        +['n','l','b','d','qty'].map(function(f){
          var _lbl={n:'N',l:'L',b:'B',d:'D',qty:'Qty'}[f];
          var _ck=!!window._msrLinkFields[f];
          return '<label style="font-size:.6rem;margin-right:.5rem;cursor:pointer;user-select:none"><input type="checkbox" onchange="msrToggleLinkField(\''+f+'\',this)" '+(_ck?'checked':'')+' style="width:12px;height:12px;vertical-align:middle;margin-right:.1rem;accent-color:#1565c0">'+_lbl+'</label>';
        }).join('')
        +'<span style="font-size:.56rem;color:var(--mu);margin-right:.2rem">× %</span>'
        +'<input type="number" step="any" value="'+(window._msrLinkPct!=null?window._msrLinkPct:100)+'" oninput="msrSetLinkPct(this)" title="Paste keleli qty = source qty × ya %" style="width:48px;font-size:.6rem;padding:.05rem .15rem;margin-right:.5rem">'
      ):'')
      ...
      +'<button onclick="msrAddPctRow('+i+')" title="Add a % row — its qty is added to this item total" style="font-size:.62rem;padding:.18rem .5rem;background:#6a1b9a;color:#fff;border:none;border-radius:3px;cursor:pointer;margin-left:.3rem">% Row</button>'
  ...
  if(!tbodiesMS && !h){
    ...
    /* Last item + Stamp — एकाच tbody मध्ये → page-break-inside:avoid
       NOTE: items.length is always >=1 here (function returns early above when empty),
       so this branch must always run to include the last item's rows — it must NOT
       be gated on tbodiesMS being non-empty, otherwise a newly-added item that happens
       to be the *last* item in the array (e.g. the very first item added, or the
       latest item added) never gets appended and silently disappears from this sheet
       even though BOQ/Rate Analysis (which don't have this gating bug) show it fine. */
  ...
  /* Post-render: update item totals that have linked rows or % rows */
  items.forEach(function(it2,ii){
    if(!it2.rows)return;
    var need=it2.rows.some(function(r){return r._linkedFrom||r.isPct;});
    if(!need)return;
    msrRecalcItem(ii);
    items[ii]=recalcItem(items[ii]);
    var totEl=R('mstot_'+ii);if(totEl)totEl.textContent=items[ii].qty.toFixed(3);
    var lblEl=R('mstotlbl_'+ii);if(lblEl)lblEl.innerHTML=msrTotalLabel(items[ii]);
  });
}
...
function rSum(){
  ...
  if(lFEl){
    var _lfTxt='+ Area '+S.areaPct+'% ('+S.areaLbl.substring(0,22)+')';
    if(S.areaPct2>0)_lfTxt+=' + '+S.areaPct2+'% ('+S.areaLbl2.substring(0,22)+')';
    lFEl.textContent=_lfTxt;
  }
...
/* Auto-versioning: jar toch nm (Name of Work) aadhich saved astel,
   tar juna overwrite na karta "<name> (v2)", "(v3)" ase navin naav
   deun navin entry mhanun save karto. Juna estimate tasach rahto.
   FAKT actual data madhe FARAK asel tarach navin version — kahich
   badal nasel tar tich version silently update hote (no v3,v4...). */
function _atpNextVersionKey(d,baseName){
  var root=(baseName||'').replace(/\s*\(v\d+\)\s*$/i,'').trim();
  if(!d[baseName])return baseName;
  var esc=root.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
  var re=new RegExp('^'+esc+'\\s*\\(v(\\d+)\\)$','i');
  var maxV=1;
  Object.keys(d).forEach(function(k){
    var m=k.match(re);
    if(m){var v=parseInt(m[1],10)||1;if(v>maxV)maxV=v;}
    else if(k===root && maxV<1){maxV=1;}
  });
  return root+' (v'+(maxV+1)+')';
}
/* Comparable data-only subset of a saved entry (dt/nm/baseName ignore —
   te version-agnostic astat, actual content nahi) */
function _atpComparablePayload(entry){
  if(!entry)return null;
  return JSON.stringify({
    meta:entry.meta,lkm:entry.lkm,leadKm:entry.leadKm,leadLoc:entry.leadLoc,
    mtfCosts:entry.mtfCosts,mtfCustom:entry.mtfCustom,mtfDesc:entry.mtfDesc,
    extraRows:entry.extraRows,steelRows:entry.steelRows,scSecs:entry.scSecs,
    planData:entry.planData,leadMapData:entry.leadMapData,items:entry.items
  });
}
function saveEstimate(){
  ...
  var mtfCustomSave=mtfCustomRowsForSave();
  var mtfDescSave=mtfDescOverridesForSave();
  ...
  var _newEntry={nm:nm,baseName:nm,dt:new Date().toLocaleDateString('en-IN'),
    ...
    lkm:lkm,leadKm:lkmFull,leadLoc:llocFull,mtfCosts:mtfSave,mtfCustom:mtfCustomSave,mtfDesc:mtfDescSave,
    extraRows:extraRowsSave,steelRows:steelRowsSave,scSecs:scSecsSave,
    planData:planDataSave, leadMapData:leadMapDataSave,
    items:items};
  /* Target = ha estimate ज्याच्याशी compare karaycha (yach session madhe
     loaded/save kelela version, kinva toch naav aadhich save asel tar to) */
  var _targetKey=null;
  if(window.CUR_SAVE_KEY&&d[window.CUR_SAVE_KEY]&&(d[window.CUR_SAVE_KEY].baseName||d[window.CUR_SAVE_KEY].nm)===nm){
    _targetKey=window.CUR_SAVE_KEY;
  } else if(d[nm]){
    _targetKey=nm;
  }
  var _unchanged=!!(_targetKey&&_atpComparablePayload(d[_targetKey])===_atpComparablePayload(_newEntry));
  var isNewVersion=false,saveKey;
  if(_targetKey&&_unchanged){
    saveKey=_targetKey; /* kahich badal nahi — tich version update (timestamp refresh) */
  } else if(_targetKey){
    saveKey=_atpNextVersionKey(d,nm); /* khara badal aahe — navin version */
    isNewVersion=true;
  } else {
    saveKey=nm; /* pahilyanda save */
  }
  _newEntry.nm=saveKey;
  d[saveKey]=_newEntry;
  putStore(d);renderSaveList();
  window.CUR_SAVE_KEY=saveKey;
  if(typeof CU!=="undefined"&&CU&&window.firebase&&firebase.firestore){
    var _cov={pName:d[saveKey].meta.pName,pSub:d[saveKey].meta.pSub,pDiv:d[saveKey].meta.pDiv,pCir:d[saveKey].meta.pCir,pReg:d[saveKey].meta.pReg};
    ...
    var _cdoc={name:saveKey,baseName:nm,uid:CU.uid,cover:_cov,items:items,grand:_grand,
      lkm:lkm,leadKm:lkmFull,leadLoc:llocFull,mtfCosts:mtfSave,mtfCustom:mtfCustomSave,mtfDesc:mtfDescSave,
      extraRows:extraRowsSave,steelRows:steelRowsSave,scSecs:scSecsSave,
      planData:planDataSave, leadMapData:leadMapDataSave,
      settings:{gst:pf('sGST'),cont:pf('sCont'),li:pf('sLI'),royS:pf('sRS'),royC:pf('sRC'),
        cancelScada:(CANCEL_SCADA||false),customScadaVal:(CUSTOM_SCADA_VAL||-126)},
      updatedAt:firebase.firestore.FieldValue.serverTimestamp()};
    firebase.firestore().collection("estimates").where("name","==",saveKey).where("uid","==",CU.uid).limit(1).get()
      .then(function(_s){return _s.empty?firebase.firestore().collection("estimates").add(_cdoc):_s.docs[0].ref.update(_cdoc);})
      .then(function(){showToast(isNewVersion?("\u2705 Saved as new version: \""+saveKey+"\" (local & cloud)"):"\u2705 Saved locally & to cloud","success");})
      .catch(function(){showToast("\u2705 Saved locally","success");});
  } else {
    showToast(isNewVersion?("\u2705 Saved as new version: \""+saveKey+"\""):("\u2705 Estimate saved: \""+nm+"\""),"success");
  }
}
function loadEstimate(nm){
  var d=getStore();var est=d[nm];if(!est)return;var m=est.meta||{};
  window.CUR_SAVE_KEY=nm; /* pudhchi save yach version chain madhe hoil */
  ...
  /* Restore custom test rows (added via + Add Test Row), then apply costs */
  mtfRestoreCustom(est.mtfCustom);
  ...
  if(est.mtfDesc&&typeof mtfData!=='undefined'){for(var _ddk in est.mtfDesc){var _ddr=mtfData.find(function(r){return r.sr===parseInt(_ddk)||r.sr===_ddk;});if(_ddr&&!_ddr.isCustom)_ddr.desc=est.mtfDesc[_ddk];}}
...
var scTypes={
  Footing:{
    ...
    diagram:'<svg viewBox="0 0 340 135" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:380px;height:auto;display:block;margin:0 auto">'
      +'<rect x="20" y="20" width="130" height="80" fill="none" stroke="#2e7d32" stroke-width="2"/>'
      +'<rect x="32" y="32" width="106" height="56" fill="none" stroke="#9e9e9e" stroke-width="1" stroke-dasharray="3,2"/>'
      +'<line x1="32" y1="42" x2="138" y2="42" stroke="#1565c0" stroke-width="1.4"/>'
      +'<line x1="32" y1="56" x2="138" y2="56" stroke="#1565c0" stroke-width="1.4"/>'
      +'<line x1="32" y1="70" x2="138" y2="70" stroke="#1565c0" stroke-width="1.4"/>'
      +'<line x1="32" y1="84" x2="138" y2="84" stroke="#1565c0" stroke-width="1.4"/>'
      +'<line x1="48" y1="32" x2="48" y2="88" stroke="#e65100" stroke-width="1.4"/>'
      +'<line x1="68" y1="32" x2="68" y2="88" stroke="#e65100" stroke-width="1.4"/>'
      +'<line x1="88" y1="32" x2="88" y2="88" stroke="#e65100" stroke-width="1.4"/>'
      +'<line x1="108" y1="32" x2="108" y2="88" stroke="#e65100" stroke-width="1.4"/>'
      +'<line x1="128" y1="32" x2="128" y2="88" stroke="#e65100" stroke-width="1.4"/>'
      +'<line x1="20" y1="14" x2="150" y2="14" stroke="#333" stroke-width="0.8"/>'
      +'<text x="85" y="11" font-size="7.5" text-anchor="middle" fill="#333">Length (excavation)</text>'
      +'<line x1="12" y1="20" x2="12" y2="100" stroke="#333" stroke-width="0.8"/>'
      +'<text x="12" y="60" font-size="7.5" text-anchor="middle" fill="#333" transform="rotate(-90 12 60)">Width</text>'
      +'<rect x="8" y="112" width="7" height="7" fill="#1565c0"/>'
      +'<text x="18" y="118" font-size="7" fill="#333">X-dir bars — Main Dia X / Spacing X</text>'
      +'<rect x="8" y="124" width="7" height="7" fill="#e65100"/>'
      +'<text x="18" y="130" font-size="7" fill="#333">Y-dir bars — Main Dia Y / Spacing Y (dashed = effective area after Cover)</text>'
      +'<rect x="215" y="40" width="35" height="55" fill="none" stroke="#2e7d32" stroke-width="2"/>'
      +'<line x1="232" y1="40" x2="232" y2="18" stroke="#c62828" stroke-width="2"/>'
      +'<circle cx="232" cy="18" r="1.8" fill="#c62828"/>'
      +'<text x="255" y="20" font-size="6.8" fill="#c62828">Dowel bar</text>'
      +'<text x="255" y="29" font-size="6.2" fill="#888">(Extra ht = 50 × dowel dia)</text>'
      +'<line x1="256" y1="40" x2="256" y2="95" stroke="#333" stroke-width="0.8"/>'
      +'<text x="260" y="70" font-size="7.5" fill="#333">Depth</text>'
      +'<line x1="215" y1="100" x2="250" y2="100" stroke="#333" stroke-width="0.8"/>'
      +'<text x="232" y="110" font-size="7" text-anchor="middle" fill="#333">Cover</text>'
      +'</svg>',
    fields:[
      {k:'nos',lbl:'Number of Footings',def:4,step:1},
      {k:'len',lbl:'Length - excavation (mm)',def:750,step:10},
      {k:'wid',lbl:'Width - excavation (mm)',def:500,step:10},
      {k:'cover',lbl:'Cover (mm)',def:50,step:5},
      {k:'depth',lbl:'Depth - excavation (mm)',def:1500,step:10},
      {k:'mainDiaX',lbl:'Main Dia X direction (mm)',def:16,step:1},
      {k:'mainSpcX',lbl:'Main Spacing X (mm)',def:120,step:10},
      {k:'mainDiaY',lbl:'Main Dia Y direction (mm)',def:12,step:1},
      {k:'mainSpcY',lbl:'Main Spacing Y (mm)',def:100,step:10},
      {k:'dowelDia',lbl:'Dowel bar dia (mm)',def:10,step:1},
      {k:'dowelNos',lbl:'Number of dowel bars',def:2,step:1}
    ],
    calc:function(v){
      /* Excel cell mapping (new sheet supplied by user) */
      var E2=v.nos||0, E3=v.len||0, E4=v.wid||0, E5=v.cover||50, E6=v.depth||0;
      var E7=v.mainDiaX||16, E8=v.mainSpcX||120, E9=v.mainDiaY||12, E10=v.mainSpcY||100;
      var E16=v.dowelDia||10, E17=v.dowelNos||2, E18=50*E16;

      /* Excel formulas EXACT */
      var G3 = E3 - 2*E5;               // Effective Length
      var G4 = E4 - 2*E5;               // Effective Width
      var G5 = Math.floor(G4/E8) + 1;   // Bars in X direction nos
      var G6 = Math.floor(G3/E8) + 1;   // Bars in Y direction nos
      var G7 = (G5*G3)/1000;            // Cut length per X-bar in m
      var G8 = (G6*G4)/1000;            // Cut length per Y-bar in m
      var G10 = (E7*E7)/162;            // Kg per meter for X dia
      var G11 = G10*G7;                 // Total steel weight per footing kg X bar
      var G12 = (E9*E9)/162;            // Kg per meter for Y dia
      var G13 = G12*G8;                 // Total steel weight per footing kg Y bar
      var G14 = (G11+G13)*E2;           // Total steel for all footings kg

      var G16 = (E16*E16)/162;          // Kg per meter for dowel dia
      var G17 = (E6+E18)/1000;          // Length of one dowel m
      var G18 = G17*G16;                // (per sheet) used as weight of one dowel
      var G19 = G18*E17;                // Total weight of dowels per footing kg
      var G20 = G19*E2;                 // Total dowel steel for all footings kg

      var G22 = G20 + G14;              // Total steel for footing

      return{rows:[
        {l:'Effective Length (mm)',v:G3.toFixed(0)},
        {l:'Effective Width (mm)',v:G4.toFixed(0)},
        {l:'Bars in X direction nos',v:G5},
        {l:'Bars in Y direction nos',v:G6},
        {l:'Cut length per X-bar (m)',v:G7.toFixed(2)},
        {l:'Cut length per Y-bar (m)',v:G8.toFixed(2)},
        {l:'Kg per meter for X dia',v:G10.toFixed(6)},
        {l:'Total steel weight per footing kg (X bar)',v:G11.toFixed(4)},
        {l:'Kg per meter for Y dia',v:G12.toFixed(6)},
        {l:'Total steel weight per footing kg (Y bar)',v:G13.toFixed(4)},
        {l:'Total steel for all footings kg (main bars)',v:G14.toFixed(4)},
        {l:'Extra height for dowel bar (mm)',v:E18.toFixed(0)},
        {l:'Kg per meter for dowel dia',v:G16.toFixed(6)},
        {l:'Length of one dowel (m)',v:G17.toFixed(3)},
        {l:'Weight of dowels per footing (kg)',v:G19.toFixed(4)},
        {l:'Total dowel steel for all footings kg',v:G20.toFixed(4)},
        {l:'Total steel for footing (kg)',v:G22.toFixed(4)}
      ],grand:G22};
    }
  },
  Column:{
    color:'#b71c1c',bg:'#ffebee',bc:'#f44336',
    diagram:'<svg viewBox="0 0 340 150" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:380px;height:auto;display:block;margin:0 auto">'
      +'<rect x="30" y="30" width="90" height="70" fill="none" stroke="#f44336" stroke-width="2"/>'
      +'<rect x="38" y="38" width="74" height="54" fill="none" stroke="#c62828" stroke-width="1.4"/>'
      +'<circle cx="38" cy="38" r="3" fill="#1565c0"/><circle cx="112" cy="38" r="3" fill="#1565c0"/>'
      +'<circle cx="38" cy="92" r="3" fill="#1565c0"/><circle cx="112" cy="92" r="3" fill="#1565c0"/>'
      +'<circle cx="75" cy="38" r="2.6" fill="#e65100"/><circle cx="75" cy="92" r="2.6" fill="#e65100"/>'
      +'<line x1="30" y1="108" x2="120" y2="108" stroke="#333" stroke-width="0.8"/>'
      +'<text x="75" y="118" font-size="7.5" text-anchor="middle" fill="#333">Column width b</text>'
      +'<line x1="16" y1="30" x2="16" y2="100" stroke="#333" stroke-width="0.8"/>'
      +'<text x="16" y="65" font-size="7.5" text-anchor="middle" fill="#333" transform="rotate(-90 16 65)">Depth h</text>'
      +'<circle cx="12" cy="128" r="3" fill="#1565c0"/><text x="20" y="131" font-size="7" fill="#333">Main bar dia1 / nos1 (corners)</text>'
      +'<circle cx="12" cy="139" r="2.6" fill="#e65100"/><text x="20" y="142" font-size="7" fill="#333">Main bar dia2 / nos2 (mid-face)</text>'
      +'<rect x="205" y="15" width="45" height="115" fill="none" stroke="#f44336" stroke-width="2"/>'
      +'<line x1="212" y1="20" x2="212" y2="125" stroke="#1565c0" stroke-width="1.2"/>'
      +'<line x1="243" y1="20" x2="243" y2="125" stroke="#1565c0" stroke-width="1.2"/>'
      +'<line x1="205" y1="30" x2="250" y2="30" stroke="#c62828" stroke-width="1.2"/>'
      +'<line x1="205" y1="52" x2="250" y2="52" stroke="#c62828" stroke-width="1.2"/>'
      +'<line x1="205" y1="74" x2="250" y2="74" stroke="#c62828" stroke-width="1.2"/>'
      +'<line x1="205" y1="96" x2="250" y2="96" stroke="#c62828" stroke-width="1.2"/>'
      +'<line x1="205" y1="118" x2="250" y2="118" stroke="#c62828" stroke-width="1.2"/>'
      +'<text x="255" y="42" font-size="6.8" fill="#c62828">Stirrup spacing s</text>'
      +'<line x1="262" y1="15" x2="262" y2="130" stroke="#333" stroke-width="0.8"/>'
      +'<text x="266" y="75" font-size="7.5" fill="#333">Clear Height H</text>'
      +'<text x="205" y="142" font-size="6.2" fill="#888">Elevation — stirrup ties every "s" mm</text>'
      +'</svg>',
    fields:[
      ...
      {k:'mainDia1',lbl:'Main bar dia1 (mm)',def:16,step:1},
      {k:'mainNos1',lbl:'Number of main bars_1',def:4,step:1},
      {k:'mainDia2',lbl:'Main bar dia2 (mm)',def:12,step:1},
      {k:'mainNos2',lbl:'Number of main bars_2',def:4,step:1},
    ...
    calc:function(v){
      /* Excel cell mapping (new sheet supplied by user - 2 main bar types) */
      var E25=v.nos||0, E26=v.width||0, E27=v.depth||0, E28=v.height||0;
      var E29=v.cover||40, E30=v.mainDia1||16, E31=v.mainNos1||4, E32=v.mainDia2||12;
      var E33=v.mainNos2||4, E34=v.stirDia||8, E35=v.stirSpc||150, E36=2*9*E34, E37=v.devLen||500;

      /* Excel formulas EXACT */
      var G25 = E26 - 2*E29;             // b_clear
      var G26 = E27 - 2*E29;             // h_clear
      var G27 = 2*(G25 + G26)/1000;      // P_st
      var G28 = E36/1000;                // hook_m
      var G29 = G27 + G28;               // L_st_one
      var G30 = Math.floor(E28/E35) + 1; // n_st
      var G31 = G29 * G30;               // L_st_total
      var G32 = 0.0061685 * E34 * E34;   // kg/m stirrup
      var G33 = G31 * G32;               // wt_stir per column (total stirrup length x kg/m)
      var G34 = G33 * E25;               // wt_stir_all
      var G35 = (E28 + E37)/1000;        // L_main_one
      var G36 = G35 * (E31 + E33);       // L_main_total (both bar types combined)
      var G37 = 0.0061685 * E30 * E30;   // kg/m main bar_1
      var G38 = G36 * G37;               // wt main bar_1
      var G39 = 0.0061685 * E32 * E32;   // kg/m main bar_2
      var G40 = G39 * G36;               // wt main bar_2
      var G41 = (G40 + G38) * E25;       // Total main bars weight all columns
      var G42 = G33 + (G40 + G38);       // Total steel per column (stirrups + main bars_1 + main bars_2)
      var G43 = (G33 + G42) * E25;       // Total steel for all columns (per sheet formula)

      return{rows:[
        {l:'Effective width b_clear (mm)',v:G25.toFixed(0)},
        {l:'Effective depth h_clear (mm)',v:G26.toFixed(0)},
        {l:'Perimeter of stirrup P_st (m)',v:G27.toFixed(3)},
        {l:'Hook length per stirrup (m)',v:G28.toFixed(3)},
        {l:'Length of one stirrup L_st_one (m)',v:G29.toFixed(3)},
        {l:'Number of stirrups per column n_st',v:G30},
        {l:'Total stirrup length per column (m)',v:G31.toFixed(3)},
        {l:'Kg per meter for stirrup (kg/m)',v:G32.toFixed(6)},
        {l:'Weight of stirrups per column (kg)',v:G33.toFixed(6)},
        {l:'Total stirrups weight for all columns (kg)',v:G34.toFixed(4)},
        {l:'Length of one main bar (m)',v:G35.toFixed(2)},
        {l:'Total main bar length per column (m)',v:G36.toFixed(2)},
        {l:'Kg per meter for main bar_1 (kg/m)',v:G37.toFixed(6)},
        {l:'Weight of main bars_1 per column (kg)',v:G38.toFixed(6)},
        {l:'Kg per meter for main bar_2 (kg/m)',v:G39.toFixed(6)},
        {l:'Weight of main bars_2 per column (kg)',v:G40.toFixed(6)},
        {l:'Total main bars weight for all columns (kg)',v:G41.toFixed(4)},
        {l:'Total steel per column (kg)',v:G42.toFixed(6)},
        {l:'Total steel for all columns (kg)',v:G43.toFixed(4)}
      ],grand:G43};
    }
  },
  Beam:{
    color:'#1565c0',bg:'#e3f2fd',bc:'#2196f3',
    diagram:'<svg viewBox="0 0 340 140" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:380px;height:auto;display:block;margin:0 auto">'
      +'<rect x="20" y="40" width="260" height="45" fill="none" stroke="#2196f3" stroke-width="2"/>'
      +'<line x1="40" y1="40" x2="40" y2="85" stroke="#e65100" stroke-width="1.1"/>'
      +'<line x1="80" y1="40" x2="80" y2="85" stroke="#e65100" stroke-width="1.1"/>'
      +'<line x1="120" y1="40" x2="120" y2="85" stroke="#e65100" stroke-width="1.1"/>'
      +'<line x1="160" y1="40" x2="160" y2="85" stroke="#e65100" stroke-width="1.1"/>'
      +'<line x1="200" y1="40" x2="200" y2="85" stroke="#e65100" stroke-width="1.1"/>'
      +'<line x1="240" y1="40" x2="240" y2="85" stroke="#e65100" stroke-width="1.1"/>'
      +'<circle cx="35" cy="79" r="2.6" fill="#1565c0"/><circle cx="88" cy="79" r="2.6" fill="#1565c0"/>'
      +'<circle cx="141" cy="79" r="2.6" fill="#1565c0"/><circle cx="194" cy="79" r="2.6" fill="#1565c0"/>'
      +'<circle cx="247" cy="79" r="2.6" fill="#1565c0"/>'
      +'<circle cx="70" cy="46" r="2.6" fill="#e65100"/><circle cx="230" cy="46" r="2.6" fill="#e65100"/>'
      +'<line x1="20" y1="30" x2="280" y2="30" stroke="#333" stroke-width="0.8"/>'
      +'<text x="150" y="27" font-size="7.5" text-anchor="middle" fill="#333">Span length L</text>'
      +'<line x1="288" y1="40" x2="288" y2="85" stroke="#333" stroke-width="0.8"/>'
      +'<text x="292" y="65" font-size="7.5" fill="#333">Beam depth h</text>'
      +'<circle cx="10" cy="102" r="2.6" fill="#1565c0"/><text x="18" y="105" font-size="7" fill="#333">Bottom bars — dia / nos (near cover)</text>'
      +'<circle cx="10" cy="113" r="2.6" fill="#e65100"/><text x="18" y="116" font-size="7" fill="#333">Top bars — dia / nos</text>'
      +'<line x1="10" y1="123" x2="20" y2="123" stroke="#e65100" stroke-width="1.4"/><text x="24" y="126" font-size="7" fill="#333">Stirrup legs @ spacing s (dia, spacing)</text>'
      +'<text x="20" y="136" font-size="6.2" fill="#888">Beam width b = cross-section width (perpendicular to page)</text>'
      +'</svg>',
    fields:[
  ...
  Slab:{
    ...
    diagram:'<svg viewBox="0 0 340 140" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:380px;height:auto;display:block;margin:0 auto">'
      +'<rect x="20" y="20" width="200" height="100" fill="none" stroke="#9c27b0" stroke-width="2"/>'
      +'<line x1="20" y1="37" x2="220" y2="37" stroke="#1565c0" stroke-width="1.2"/>'
      +'<line x1="20" y1="54" x2="220" y2="54" stroke="#1565c0" stroke-width="1.2"/>'
      +'<line x1="20" y1="71" x2="220" y2="71" stroke="#1565c0" stroke-width="1.2"/>'
      +'<line x1="20" y1="88" x2="220" y2="88" stroke="#1565c0" stroke-width="1.2"/>'
      +'<line x1="20" y1="105" x2="220" y2="105" stroke="#1565c0" stroke-width="1.2"/>'
      +'<line x1="45" y1="20" x2="45" y2="120" stroke="#e65100" stroke-width="1.2"/>'
      +'<line x1="80" y1="20" x2="80" y2="120" stroke="#e65100" stroke-width="1.2"/>'
      +'<line x1="115" y1="20" x2="115" y2="120" stroke="#e65100" stroke-width="1.2"/>'
      +'<line x1="150" y1="20" x2="150" y2="120" stroke="#e65100" stroke-width="1.2"/>'
      +'<line x1="185" y1="20" x2="185" y2="120" stroke="#e65100" stroke-width="1.2"/>'
      +'<line x1="20" y1="12" x2="220" y2="12" stroke="#333" stroke-width="0.8"/>'
      +'<text x="120" y="9" font-size="7.5" text-anchor="middle" fill="#333">Slab length L</text>'
      +'<line x1="12" y1="20" x2="12" y2="120" stroke="#333" stroke-width="0.8"/>'
      +'<text x="12" y="70" font-size="7.5" text-anchor="middle" fill="#333" transform="rotate(-90 12 70)">Slab width B</text>'
      +'<rect x="240" y="55" width="70" height="16" fill="none" stroke="#9c27b0" stroke-width="1.6"/>'
      +'<text x="275" y="52" font-size="7" text-anchor="middle" fill="#333">Thickness</text>'
      +'<rect x="8" y="126" width="7" height="7" fill="#1565c0"/><text x="18" y="132" font-size="7" fill="#333">Main bars — Main Dia / Main Spacing</text>'
      +'<rect x="185" y="126" width="7" height="7" fill="#e65100"/><text x="195" y="132" font-size="7" fill="#333">Distribution bars — Dist. Dia / Spacing</text>'
      +'</svg>',
    fields:[
...
function scRender(){
  ...
  scSecs.forEach(function(sec){
    ...
    h+='<div class="sc-hd noprt" style="background:'+t.bg+';border-bottom:1px solid '+t.bc+'">'
      ...
      +'</div>';
    /* Dimension legend diagram - screen only, hidden from print */
    if(t.diagram){
      h+='<div class="sc-dim noprt" style="padding:.4rem .6rem;background:#fbfbfb;border-bottom:1px solid '+t.bc+'">'+t.diagram+'</div>';
    }
    /* Print header */
  ...
  _autoSaveTimer=setTimeout(function(){
    try{
      if(typeof CU!=='undefined'&&CU&&window.firebase&&firebase.firestore&&typeof CUR_EST_ID!=='undefined'&&CUR_EST_ID){
        ...
        var mtfCustomSave2=mtfCustomRowsForSave();
        var mtfDescSave2=mtfDescOverridesForSave();
        firebase.firestore().collection('estimates').doc(CUR_EST_ID).update({
          lkm:lkm2,leadKm:lkmFull2,leadLoc:llocFull2,mtfCosts:mtfSave2,mtfCustom:mtfCustomSave2,mtfDesc:mtfDescSave2,
          steelRows:(typeof steelRows!=='undefined'?JSON.parse(JSON.stringify(steelRows)):[]),
          scSecs:(typeof scSecs!=='undefined'?JSON.parse(JSON.stringify(scSecs)):[]),
          updatedAt:firebase.firestore.FieldValue.serverTimestamp()
...
function _mtfEscHtml(s){return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}
var MTF_CHART_IMAGE=''; /* single reference chart PDF URL, admin-set, shown below Testing table (never in print) */
...
function resetMTF(){
  ...
  mtfData.forEach(function(row){
    ...
    if(row.isCustom){
      h+='<td style="font-size:.57rem;white-space:nowrap"><button class="noprt" onclick="removeMTFRow('+row.sr+')" style="background:#c0392b;color:#fff;border:none;border-radius:4px;padding:.1rem .35rem;font-size:.55rem;cursor:pointer;margin-right:.25rem">🗑</button>'+row.sr+'</td>';
    } else {
      h+='<td style="font-size:.57rem">'+row.sr+'</td>';
    }
    if(row.isCustom){
      h+='<td style="font-size:.57rem"><input class="mtf-edit" type="text" value="'+(row.mat||'').replace(/"/g,'&quot;')+'" placeholder="Material" onchange="updateMTFMat('+row.sr+',this.value)"><span class="mtf-prt" style="display:none">'+(row.mat||'')+'</span></td>';
    } else {
      h+='<td style="font-size:.57rem">'+row.mat+'</td>';
    }
    h+='<td style="font-size:.57rem"><textarea class="mtf-edit mtf-desc" rows="3" style="width:100%;font-size:.57rem;line-height:1.3;white-space:pre-line;resize:vertical;font-family:inherit;box-sizing:border-box" placeholder="Description of test" onchange="updateMTFDesc('+row.sr+',this.value)">'+_mtfEscHtml(row.desc||'')+'</textarea><span class="mtf-prt" style="display:none;white-space:pre-line">'+(row.desc||'')+'</span></td>';
...
function renderMTFRefImage(){
  var box=G('mtfRefImageBox');if(!box)return;
  if(MTF_CHART_IMAGE){
    box.style.display='block';
    box.innerHTML='<div style="font-size:.62rem;font-weight:700;color:#555;margin-bottom:.3rem">📄 Reference for Testing Charges</div>'
      +'<iframe src="'+MTF_CHART_IMAGE+'" style="width:100%;height:480px;border:1px solid #d0ccc4;border-radius:6px" title="Testing Charges Reference PDF"></iframe>'
      +'<div style="margin-top:.3rem"><a href="'+MTF_CHART_IMAGE+'" target="_blank" rel="noopener" style="font-size:.62rem;color:#1565c0;font-weight:700;text-decoration:none">🔗 Open PDF in new tab</a></div>';
  } else {
...
function updateMTFMat(sr,val){
  var row=mtfData.find(function(r){return r.sr===sr;});
  if(row)row.mat=val;
}
function updateMTFDesc(sr,val){
  var row=mtfData.find(function(r){return r.sr===sr;});
  if(row)row.desc=val;
}
function addMTFRow(){
  var maxSr=0;mtfData.forEach(function(r){if(r.sr>maxSr)maxSr=r.sr;});
  var newSr=maxSr+1;
  mtfData.push({sr:newSr,mat:'New Test',desc:'',freq:'—',ref:'Custom Entry',
    qtyFormula:'',threshold:0,cost:0,isCustom:true,_qty:1,_tests:1});
  calcMTF();_refreshGrand();
}
function removeMTFRow(sr){
  var row=mtfData.find(function(r){return r.sr===sr;});
  if(!row||!row.isCustom)return;
  if(!confirm('Remove this test row?'))return;
  mtfData=mtfData.filter(function(r){return r.sr!==sr;});
  calcMTF();_refreshGrand();
}
function mtfCustomRowsForSave(){
  if(typeof mtfData==='undefined')return[];
  return mtfData.filter(function(r){return r.isCustom;}).map(function(r){
    return{sr:r.sr,mat:r.mat,desc:r.desc,freq:r.freq,ref:r.ref,cost:r.cost,
      _qty:(r._qty!==undefined?r._qty:0),_tests:(r._tests!==undefined?r._tests:0)};
  });
}
function mtfDescOverridesForSave(){
  if(typeof mtfData==='undefined')return{};
  var out={};
  mtfData.forEach(function(r){if(!r.isCustom)out[r.sr]=r.desc;});
  return out;
}
function mtfRestoreCustom(customArr){
  if(!customArr||!customArr.length||typeof mtfData==='undefined')return;
  /* Remove any existing custom rows first, then rebuild from saved data */
  mtfData=mtfData.filter(function(r){return !r.isCustom;});
  customArr.forEach(function(c){
    mtfData.push({sr:c.sr,mat:c.mat||'New Test',desc:c.desc||'',freq:c.freq||'—',
      ref:c.ref||'Custom Entry',qtyFormula:'',threshold:0,cost:c.cost||0,isCustom:true,
      _qty:c._qty||0,_tests:c._tests||0});
  });
}

/* ── ADMIN: Default Testing Cost/Test + Reference Chart (syncs to all users) ─ */
var _mtfAdminChartImage='';
function aLoadMTFCosts(){
  var box=G('mtfAdminList');if(!box)return;
  box.innerHTML='<div style="font-size:.65rem;color:#888">Loading...</div>';
  if(!window.firebase||!firebase.firestore){renderMTFAdminInputs({},'');return;}
  firebase.firestore().collection('config').doc('mtf_costs').get()
    .then(function(doc){
      var d=(doc.exists&&doc.data())||{};
      renderMTFAdminInputs(d.costs||{},d.chartImage||'');
    })
    .catch(function(){renderMTFAdminInputs({},'');});
}
function renderMTFAdminInputs(cloudCosts,chartImage){
  var box=G('mtfAdminList');if(!box)return;
  var stdRows=mtfData.filter(function(r){return!r.isCustom;});
  var h='';
  stdRows.forEach(function(r){
    var val=(cloudCosts[r.sr]!==undefined)?cloudCosts[r.sr]:(r.cost||0);
    h+='<div style="display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;padding:.3rem 0;border-bottom:1px solid #f0ede8">'
      +'<label style="width:130px;font-size:.68rem;font-weight:700">'+r.mat+'</label>'
      +'<input type="number" min="0" step="any" id="mtfAdm_'+r.sr+'" value="'+val+'" '
      +'style="flex:1;min-width:100px;padding:.32rem .5rem;border:1px solid #d0ccc4;border-radius:5px;font-size:.72rem">'
      +'</div>';
  });
  box.innerHTML=h;
  _mtfAdminChartImage=chartImage||'';
  renderMTFChartImageAdmin();
}
function renderMTFChartImageAdmin(){
  var wrap=G('mtfChartImgWrap');if(!wrap)return;
  if(_mtfAdminChartImage){
    wrap.innerHTML='<a href="'+_mtfAdminChartImage+'" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:.35rem;background:#c62828;color:#fff;border-radius:6px;padding:.4rem .7rem;font-size:.65rem;font-weight:700;text-decoration:none">📄 View Uploaded PDF</a>'
      +'<button onclick="aRemoveMTFChartImage()" style="background:#c0392b;color:#fff;border:none;border-radius:4px;padding:.28rem .5rem;font-size:.6rem;cursor:pointer;height:fit-content">✕ Remove</button>';
  } else {
    wrap.innerHTML='<label style="cursor:pointer;background:#1565c0;color:#fff;border-radius:5px;padding:.4rem .7rem;font-size:.65rem;font-weight:700">📄 Upload Reference PDF'
      +'<input type="file" accept="application/pdf" style="display:none" onchange="aUploadMTFChartImage(this)"></label>';
  }
}
function aUploadMTFChartImage(input){
  ...
  if(!window.firebase||!firebase.firestore){showToast('Firebase not ready','error');return;}
  var wrap=G('mtfChartImgWrap');
  if(wrap)wrap.innerHTML='<span style="font-size:.6rem;color:#1565c0">Uploading...</span>';
  uploadCDN(f,'atp-maha-pwd/mtf-images',
    function(){},
    function(url){
      _mtfAdminChartImage=url;
      firebase.firestore().collection('config').doc('mtf_costs').set(
        {chartImage:url,updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true}
      ).then(function(){
        renderMTFChartImageAdmin();
        if(typeof showToast==='function')showToast('☁️ Reference PDF saved','success');
      }).catch(function(){if(typeof showToast==='function')showToast('Save failed','error');});
    },
    function(){if(typeof showToast==='function')showToast('Upload failed','error');renderMTFChartImageAdmin();}
  );
}
function aRemoveMTFChartImage(){
  if(!window.firebase||!firebase.firestore)return;
  _mtfAdminChartImage='';
  firebase.firestore().collection('config').doc('mtf_costs').set(
    {chartImage:firebase.firestore.FieldValue.delete(),updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true}
  ).then(function(){renderMTFChartImageAdmin();}).catch(function(){});
}
function aSaveMTFCosts(){
  if(!window.firebase||!firebase.firestore){showToast('Firebase not ready','error');return;}
  var stdRows=mtfData.filter(function(r){return!r.isCustom;});
  var costs={};
  stdRows.forEach(function(r){
    var el=G('mtfAdm_'+r.sr);
    costs[r.sr]=el?(parseFloat(el.value)||0):(r.cost||0);
  });
  firebase.firestore().collection('config').doc('mtf_costs').set({
    costs:costs,updatedAt:firebase.firestore.FieldValue.serverTimestamp()
  },{merge:true}).then(function(){
    var m=G('mtfAdminMsg');if(m)m.textContent='✅ Saved — new users will see these defaults.';
    if(typeof showToast==='function')showToast('☁️ Testing prices saved','success');
  }).catch(function(){
    var m=G('mtfAdminMsg');if(m)m.textContent='';
    if(typeof showToast==='function')showToast('Save failed','error');
  });
}
function loadMTFDefaultsFromCloud(){
  if(!window.firebase||!firebase.firestore)return;
  firebase.firestore().collection('config').doc('mtf_costs').get().then(function(doc){
    if(!doc.exists)return;
    var d=doc.data()||{};
    var costs=d.costs||{};
    MTF_CHART_IMAGE=d.chartImage||'';
    var changed=false;
    mtfData.forEach(function(r){
      if(r.isCustom)return;
      /* Only apply the cloud default if the user hasn't already set a cost this session */
      if((!r.cost||r.cost===0)&&costs[r.sr]!==undefined){r.cost=parseFloat(costs[r.sr])||0;changed=true;}
    });
    calcMTF();
    if(changed&&typeof _refreshGrand==='function')_refreshGrand();
  }).catch(function(){});
}
...
setInterval(function(){
  try{
    ...
    var _mtfCustom=mtfCustomRowsForSave();
    var _mtfDesc=mtfDescOverridesForSave();
    d['__autosave__']={nm:nm,dt:new Date().toLocaleDateString('en-IN'),_auto:true,
      meta:{pName:((R('pName')||{}).value||''),pSub:((R('pSub')||{}).value||''),
        ...
        sGST:pf('sGST'),sCont:pf('sCont'),sLI:pf('sLI'),sRS:pf('sRS'),sRC:pf('sRC'),
        cancelScada:(CANCEL_SCADA||false),customScadaVal:(CUSTOM_SCADA_VAL||-126)},
      lkm:lkm,leadKm:_lkf,mtfCosts:_mtfCosts,mtfCustom:_mtfCustom,mtfDesc:_mtfDesc,
      steelRows:(typeof steelRows!=='undefined'?JSON.parse(JSON.stringify(steelRows)):[]),
      scSecs:(typeof scSecs!=='undefined'?JSON.parse(JSON.stringify(scSecs)):[]),
      items:items};
...
scRender();
calcMTF();
loadMTFDefaultsFromCloud(); /* fetch admin-set default testing prices */

/* ── ADMIN: Bitumen Rate Reference (structured table + optional PDF; syncs to all users) ─ */
var BIT_REF_DATA={date:'',ssrLabel:'',rows:[],pdfURL:''};
var _bitRefAdminData={date:'',ssrLabel:'',rows:[],pdfURL:''};
var BIT_DEFAULT_ROWS=[
  {product:'Btmn VG 10 Bulk',unit:'MT',basic:0,ssr:0},
  {product:'Btmn VG 30 Bulk',unit:'MT',basic:0,ssr:0},
  {product:'Btmn VG 40 Bulk',unit:'MT',basic:0,ssr:0},
  {product:'Btmn VG 10 Pack',unit:'MT',basic:0,ssr:0},
  {product:'Btmn VG 30 Pack',unit:'MT',basic:0,ssr:0},
  {product:'Btmn VG 40 Pack',unit:'MT',basic:0,ssr:0}
];
function aLoadBitRefImage(){
  var body=G('bitRefAdminBody');if(!body)return;
  body.innerHTML='<tr><td colspan="5" style="text-align:center;color:#888;padding:.6rem">Loading...</td></tr>';
  if(!window.firebase||!firebase.firestore){_bitRefAdminData={date:'',ssrLabel:'',rows:BIT_DEFAULT_ROWS.slice(),pdfURL:''};renderBitRefAdminTable();renderBitRefPdfAdmin();return;}
  firebase.firestore().collection('config').doc('bitumen_rate').get()
    .then(function(doc){
      var d=(doc.exists&&doc.data())||{};
      _bitRefAdminData={
        date:d.date||'',
        ssrLabel:d.ssrLabel||'',
        rows:(d.rows&&d.rows.length)?d.rows:BIT_DEFAULT_ROWS.slice(),
        pdfURL:d.pdfURL||''
      };
      if(G('bitRefDate'))G('bitRefDate').value=_bitRefAdminData.date;
      if(G('bitRefSSRLabel'))G('bitRefSSRLabel').value=_bitRefAdminData.ssrLabel;
      renderBitRefAdminTable();
      renderBitRefPdfAdmin();
    })
    .catch(function(){_bitRefAdminData={date:'',ssrLabel:'',rows:BIT_DEFAULT_ROWS.slice(),pdfURL:''};renderBitRefAdminTable();renderBitRefPdfAdmin();});
}
function renderBitRefAdminTable(){
  var body=G('bitRefAdminBody');if(!body)return;
  var rows=_bitRefAdminData.rows||[];
  if(!rows.length){body.innerHTML='<tr><td colspan="5" style="text-align:center;color:#aaa;padding:.6rem">No rows — tap + Add Row</td></tr>';return;}
  body.innerHTML=rows.map(function(r,i){
    return '<tr>'
      +'<td style="padding:.25rem .3rem"><input value="'+escAttr(r.product||'')+'" oninput="bitRefUpd('+i+',\'product\',this.value)" style="width:100%;padding:.25rem .35rem;border:1px solid #d0ccc4;border-radius:4px;font-size:.63rem;box-sizing:border-box"></td>'
      +'<td style="padding:.25rem .3rem"><input value="'+escAttr(r.unit||'')+'" oninput="bitRefUpd('+i+',\'unit\',this.value)" style="width:100%;padding:.25rem .35rem;border:1px solid #d0ccc4;border-radius:4px;font-size:.63rem;text-align:center;box-sizing:border-box"></td>'
      +'<td style="padding:.25rem .3rem"><input type="number" value="'+(r.basic!=null?r.basic:'')+'" oninput="bitRefUpd('+i+',\'basic\',this.value)" style="width:100%;padding:.25rem .35rem;border:1px solid #d0ccc4;border-radius:4px;font-size:.63rem;text-align:center;box-sizing:border-box"></td>'
      +'<td style="padding:.25rem .3rem"><input type="number" value="'+(r.ssr!=null?r.ssr:'')+'" oninput="bitRefUpd('+i+',\'ssr\',this.value)" style="width:100%;padding:.25rem .35rem;border:1px solid #d0ccc4;border-radius:4px;font-size:.63rem;text-align:center;box-sizing:border-box"></td>'
      +'<td style="text-align:center"><button onclick="aDelBitRefRow('+i+')" style="background:#c0392b;color:#fff;border:none;border-radius:4px;width:22px;height:22px;font-size:.62rem;cursor:pointer">✕</button></td>'
      +'</tr>';
  }).join('');
}
function bitRefUpd(i,field,val){
  if(!_bitRefAdminData.rows[i])return;
  _bitRefAdminData.rows[i][field]=(field==='basic'||field==='ssr')?(parseFloat(val)||0):val;
}
function aAddBitRefRow(){
  _bitRefAdminData.rows=_bitRefAdminData.rows||[];
  _bitRefAdminData.rows.push({product:'',unit:'MT',basic:0,ssr:0});
  renderBitRefAdminTable();
}
function aDelBitRefRow(i){
  _bitRefAdminData.rows.splice(i,1);
  renderBitRefAdminTable();
}
function renderBitRefPdfAdmin(){
  var wrap=G('bitRefPdfWrap');if(!wrap)return;
  if(_bitRefAdminData.pdfURL){
    wrap.innerHTML='<a href="'+_bitRefAdminData.pdfURL+'" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:.35rem;background:#c62828;color:#fff;border-radius:6px;padding:.4rem .7rem;font-size:.65rem;font-weight:700;text-decoration:none">📄 View Uploaded PDF</a>'
      +'<button onclick="aRemoveBitRefPdf()" style="background:#c0392b;color:#fff;border:none;border-radius:4px;padding:.28rem .5rem;font-size:.6rem;cursor:pointer;height:fit-content">✕ Remove</button>';
  } else {
    wrap.innerHTML='<label style="cursor:pointer;background:#1565c0;color:#fff;border-radius:5px;padding:.4rem .7rem;font-size:.65rem;font-weight:700">📄 Upload PDF'
      +'<input type="file" accept="application/pdf" style="display:none" onchange="aUploadBitRefPdf(this)"></label>';
  }
}
function aUploadBitRefPdf(input){
  if(!input.files||!input.files[0])return;
  var f=input.files[0];
  var isPdf=(f.type==='application/pdf')||/\.pdf$/i.test(f.name||'');
  if(!isPdf){if(typeof showToast==='function')showToast('Please upload a PDF file','error');input.value='';return;}
  if(!window.firebase||!firebase.firestore){if(typeof showToast==='function')showToast('Firebase not ready','error');return;}
  var wrap=G('bitRefPdfWrap');
  if(wrap)wrap.innerHTML='<span style="font-size:.6rem;color:#1565c0">Uploading...</span>';
  uploadCDN(f,'atp-maha-pwd/bitumen-rate-pdf',
    function(){},
    function(url){
      _bitRefAdminData.pdfURL=url;
      renderBitRefPdfAdmin();
      if(typeof showToast==='function')showToast('PDF ready — tap ☁️ Save to Cloud to publish','success');
    },
    function(){if(typeof showToast==='function')showToast('Upload failed','error');renderBitRefPdfAdmin();}
  );
}
function aRemoveBitRefPdf(){
  _bitRefAdminData.pdfURL='';
  renderBitRefPdfAdmin();
}
function aSaveBitRefData(){
  if(!window.firebase||!firebase.firestore){if(typeof showToast==='function')showToast('Firebase not ready','error');return;}
  _bitRefAdminData.date=(G('bitRefDate')?G('bitRefDate').value:'')||'';
  _bitRefAdminData.ssrLabel=(G('bitRefSSRLabel')?G('bitRefSSRLabel').value:'')||'';
  firebase.firestore().collection('config').doc('bitumen_rate').set({
    date:_bitRefAdminData.date,
    ssrLabel:_bitRefAdminData.ssrLabel,
    rows:_bitRefAdminData.rows||[],
    pdfURL:_bitRefAdminData.pdfURL||'',
    updatedAt:firebase.firestore.FieldValue.serverTimestamp()
  },{merge:true}).then(function(){
    var m=G('bitRefSaveMsg');if(m)m.textContent='✅ Saved — all users will now see this.';
    if(typeof showToast==='function')showToast('☁️ Bitumen rates saved','success');
  }).catch(function(){
    var m=G('bitRefSaveMsg');if(m)m.textContent='';
    if(typeof showToast==='function')showToast('Save failed','error');
  });
}

/* ── USER SIDE: render bitumen rate reference table ─ */
function renderBitRefImage(){
  var box=G('bitRefImageBox');if(!box)return;
  var d=BIT_REF_DATA;
  var rows=d.rows||[];
  if(!rows.length&&!d.pdfURL){box.style.display='none';box.innerHTML='';return;}
  box.style.display='block';
  var h='<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.4rem;margin-bottom:.4rem">'
    +'<div style="font-size:.62rem;font-weight:700;color:#555">🛢️ Bitumen Rates'+(d.date?' — <span style="color:#888;font-weight:600">'+esc(d.date)+'</span>':'')+'</div>'
    +(d.ssrLabel?'<span style="font-size:.6rem;font-weight:700;color:#1565c0;border:1px solid #1565c0;border-radius:20px;padding:.15rem .55rem">'+esc(d.ssrLabel)+'</span>':'')
    +'</div>';
  if(rows.length){
    h+='<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:.63rem;background:#fff;border:1px solid #e0dbd4;border-radius:6px;overflow:hidden">'
      +'<thead><tr style="background:#e8eaf6"><th style="padding:.35rem .4rem;text-align:left">Product</th><th style="padding:.35rem .4rem;text-align:center">Unit</th><th style="padding:.35rem .4rem;text-align:center">Basic (excl. GST)</th><th style="padding:.35rem .4rem;text-align:center">SSR Rate</th></tr></thead><tbody>'
      +rows.map(function(r){
        return '<tr style="border-top:1px solid #f0ede8">'
          +'<td style="padding:.35rem .4rem;font-weight:700">'+esc(r.product||'')+'</td>'
          +'<td style="padding:.35rem .4rem;text-align:center;color:#888">'+esc(r.unit||'')+'</td>'
          +'<td style="padding:.35rem .4rem;text-align:center">'+(r.basic||0)+'</td>'
          +'<td style="padding:.35rem .4rem;text-align:center;font-weight:800;color:#1a237e">'+(r.ssr||0)+'</td>'
          +'</tr>';
      }).join('')
      +'</tbody></table></div>';
  }
  if(d.pdfURL){
    h+='<div style="text-align:right;margin-top:.4rem"><a href="'+d.pdfURL+'" target="_blank" rel="noopener" style="font-size:.62rem;color:#1565c0;font-weight:700;text-decoration:none">📄 See full PDF ⬇️</a></div>';
  }
  box.innerHTML=h;
}
function loadBitRefImageFromCloud(){
  if(!window.firebase||!firebase.firestore)return;
  firebase.firestore().collection('config').doc('bitumen_rate').get().then(function(doc){
    if(!doc.exists)return;
    var d=doc.data()||{};
    BIT_REF_DATA={date:d.date||'',ssrLabel:d.ssrLabel||'',rows:d.rows||[],pdfURL:d.pdfURL||''};
    renderBitRefImage();
  }).catch(function(){});
}
loadBitRefImageFromCloud(); /* fetch admin-set bitumen rate reference */
showDefaultList();
renderSaveList();
upCover();
upMeta();
...
(function(){
  ...
  function _cloudSilent(){
    try{
      ...
      var _saveKey = window.CUR_SAVE_KEY || nm;
      var _doc={
        name:_saveKey, baseName:nm, uid:CU.uid, cover:_cov,
        items:(items||[]), grand:_grand,
        lkm:lkm, leadKm:lkmFull, leadLoc:llocFull,
        mtfCosts:mtfSave, mtfCustom:mtfCustomSave, mtfDesc:mtfDescSave, extraRows:extraRowsSave,
      ...
      firebase.firestore()
        ...
        .where('name','==',_saveKey).where('uid','==',CU.uid).limit(1).get()
        .then(function(_s){
          return _s.empty
