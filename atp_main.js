...
function calcGrand(){
  ...
  var mtf=window._mtfTotalVal||0;
  var ABC=A+roy+mtf;
  var specF=S.areaPct>0?Math.round(A*S.areaPct/100):0;
...
function smGetNames(){
  ...
  return {
    se:        localStorage.getItem('stampSecEng')||'Section Engineer',
    seTitle:   localStorage.getItem('stampSETitle')||'Section Engineer',
    sde:       localStorage.getItem('stampSubDivEng')||'Sub Divisional Engineer',
    sdeTitle:  localStorage.getItem('stampSDETitle')||'Sub Divisional Engineer',
    de:        localStorage.getItem('stampDE')||'Divisional Engineer',
    deTitle:   localStorage.getItem('stampDETitle')||'Divisional Engineer',
    ee:        localStorage.getItem('stampEE')||'Executive Engineer',
    eeTitle:   localStorage.getItem('stampEETitle')||'Executive Engineer',
    se_circle: localStorage.getItem('stampSECircle')||'Superintending Engineer',
    seCircleTitle: localStorage.getItem('stampSECircleTitle')||'Superintending Engineer',
    ce:        localStorage.getItem('stampCE')||'Chief Engineer',
...
function updateAllStamps(){
  ...
  document.querySelectorAll('.stmp-se').forEach(function(e){e.textContent=names.se;});
  document.querySelectorAll('.stmp-sde').forEach(function(e){e.textContent=names.sde;});
  document.querySelectorAll('.stmp-loc').forEach(function(e){e.textContent=names.locSub||names.loc;});
  document.querySelectorAll('.stmp-ee').forEach(function(e){e.textContent=names.ee||'Executive Engineer';});
  document.querySelectorAll('.stmp-se_circle').forEach(function(e){e.textContent=names.se_circle||'Superintending Engineer';});
  document.querySelectorAll('.stmp-ce').forEach(function(e){e.textContent=names.ce||'Chief Engineer';});
  ...
  document.querySelectorAll('.sheet-stamp-block').forEach(function(block){
    ...
    activeCfg.forEach(function(rk, ri){
      ...
      /* 1. Officer Name */
      boxes += '<div><span class="stmp-'+rk+'" data-role="'+rk+'" contenteditable="true" title="Click to edit" style="cursor:text;border-bottom:1px dashed #aaa;outline:none" onblur="smInlineEdit(this)" onkeydown="smStampKeydown(event,this)">'+r.label+'</span></div>';
      /* 2. Sign Space */
...
function spMouseUp(e){SP.dragging=null;}
...
function rBOQ(){
  ...
  for(i=0;i<items.length;i++){
    ...
    if(it.rows){
      it.rows.forEach(function(rw){
        ...
        if(fl>0)hasUpperFloor=true; /* Upper floor rows ahet */
        var q=(rw.n||1)*(rw.l||0);
  ...
  h+='<tr class="tfooter"><td colspan="7" style="text-align:right">Total =</td>'
...
function rRA(){
  ...
  var h='',i,it,desc,j,m,rowspan,subRows;
  for(i=0;i<items.length;i++){
    it=items[i];
  ...
  tb.innerHTML=h||'<tr><td colspan="6" class="ni">Add items to generate</td></tr>';
}
...
function rMS(){
  ...
  tb.innerHTML=h||'<tr><td colspan="11" class="ni">Add items to generate</td></tr>';
...
function rCC(){
  ...
  var h='',i,it,cf,desc,hasMat,j,m,f,nq;
  for(i=0;i<items.length;i++){
    it=items[i];cf=it.cf||{};
    /* Collect non-zero materials for this item */
    var itMats=[];
    for(j=0;j<MATS.length;j++){
      m=MATS[j];f=cf[m.k]||0;
      if(f>0){
        /* Factor = CF value as-is, Net Qty = Factor × Item Qty */
  ...
  tb.innerHTML=h||'<tr><td colspan="7" class="ni">Add items to generate</td></tr>';
}
...
function rRoy(){
  ...
  var tRS=0,tRC=0,h='',i,it,cf,sF,cF,nS,nC,rS,rC,desc,matRows,rowspan,j;
  for(i=0;i<items.length;i++){
    it=items[i];cf=it.cf||{};
    sF=(cf.scr_sand||0)+(cf.sand||0)+(cf.quarry||0);
    cF=(cf.m40mm||0)+(cf.m20mm||0)+(cf.m12_10||0)+(cf.overmetal||0)+(cf.rubble||0);
  ...
  if(tRS>0||tRC>0){
    h+='<tr class="tfooter"><td colspan="4" style="text-align:right">Total Sand/Quarry Royalty</td>'
  ...
  tb.innerHTML=h||'<tr><td colspan="8" style="text-align:center;padding:1.2rem;color:var(--mu)">No royalty materials in added items</td></tr>';
}

/* ── FINAL ESTIMATE ──────────────────────────────────────────────────── */
function rFE(){
  var S=gS(),tb=R('feTb');if(!tb)return;
  if(!items.length){tb.innerHTML='<tr><td colspan="3" class="ni">Add items to generate</td></tr>';return;}
  ...
  var grand=ABC+gst+cont+li+specF+extraTotalFE;
  var h='<tr><td style="text-align:right;color:var(--mu);font-size:.72rem">A.</td>'
    +'<td>Cost of Work</td>'
    +'<td class="r gr" style="font-weight:700">\u20b9'+fmt(A)+'</td></tr>'
    +'<tr><td style="text-align:right;color:var(--mu);font-size:.72rem">B.</td>'
    +'<td style="color:var(--mu)">Royalty</td>'
    +'<td class="r">\u20b9'+fmt(roy)+'</td></tr>'
    +'<tr><td style="text-align:right;color:var(--mu);font-size:.72rem">C.</td>'
    +'<td style="color:var(--mu)">Material Testing</td>'
    +'<td class="r">\u20b9'+fmt(mtf)+'</td></tr>'
    +'<tr style="background:var(--bg)">'
    +'<td colspan="2" style="text-align:right;font-weight:700">Total (A + B + C)</td>'
    +'<td class="r" style="font-weight:700">\u20b9'+fmt(ABC)+'</td></tr>'
    +'<tr style="height:5px"><td colspan="3" style="padding:0"></td></tr>'
    +'<tr><td style="text-align:right;color:var(--mu);font-size:.72rem">D.</td>'
    +'<td style="color:var(--mu)">Add '+S.gst+'% GST</td>'
    +'<td class="r">\u20b9'+fmt(gst)+'</td></tr>'
    +(S.cont>0?'<tr><td style="text-align:right;color:var(--mu);font-size:.72rem">E.</td>'
    +'<td style="color:var(--mu)">Add '+S.cont+'% Contingencies</td>'
    +'<td class="r">\u20b9'+fmt(cont)+'</td></tr>':'')
    +(S.li>0?'<tr><td style="text-align:right;color:var(--mu);font-size:.72rem">F.</td>'
    +'<td style="color:var(--mu)">Add '+S.li+'% Labour Insurance</td>'
    +'<td class="r">\u20b9'+fmt(li)+'</td></tr>':'');
  if(specF>0){
    h+='<tr style="background:#fff8e1">'
      +'<td style="text-align:right;color:#e65100;font-size:.72rem;font-weight:700">G.</td>'
      +'<td style="color:#e65100;font-size:.63rem">Add '+S.areaPct+'% for Specific Area<br>'
      +'<span style="font-size:.55rem;color:#888">('+S.areaLbl+')</span></td>'
      +'<td class="r" style="color:#e65100;font-weight:700">\u20b9'+fmt(specF)+'</td></tr>';
  }
  var letters=['H','I','J','K','L','M','N'];
  if(S.extraRows&&S.extraRows.length){
    for(var _ei2=0;_ei2<S.extraRows.length;_ei2++){
      ...
      if(_er.pct>0){
        ...
        h+='<tr style="background:#e8f5e9"><td style="text-align:right;color:#1b5e20;font-size:.72rem;font-weight:700">'+(letters[_ei2]||'X')+'.</td>'
  ...
  h+='<tr style="height:5px"><td colspan="3" style="padding:0"></td></tr>'
    +'<tr style="background:#18150f;color:#fff">'
    +'<td colspan="2" style="text-align:right;font-weight:900;font-size:.84rem;padding:.4rem .44rem">Grand Total</td>'
    +'<td class="r" style="font-weight:900;font-size:.84rem;color:#f5a623">₹'+fmtW(grand)+'</td></tr>'
    +'<tr style="background:var(--bg)"><td colspan="2" style="text-align:right;font-size:.7rem;color:var(--mu)">= Rs.</td>'
...
function rSum(){
  ...
  var grand=ABC+gst+cont+li+specF+extraTotal2;
  ...
  if(gaFRow)gaFRow.style.display=specF>0?'':' none';
...
function goTab(n){
  ...
  if(n===12&&typeof spRedraw==='function'){spRedraw();}
  if(n===13)rCFTab();
...
function saveEstimate(){
  ...
  if(typeof CU!=="undefined"&&CU&&window.firebase&&firebase.firestore){
    ...
    try{
      ...
      var _specF=_S.areaPct>0?Math.round(_A*_S.areaPct/100):0;
...
function loadEstimate(nm){
  ...
  upCover();upMeta();updateAll();alert('\u2705 Estimate loaded: "'+nm+'"');}
