...
function msrUpd(iIdx,rIdx,el,field){
  ...
  items.forEach(function(dIt,dIIdx){
    ...
    dIt.rows.forEach(function(dR,dRIdx){
      ...
      var _fList=msrLinkFieldsOf(dR);  /* only these fields mirror the source row */
      if(_fList.indexOf('n')>-1) dR.n=r.n||1;
      if(_fList.indexOf('l')>-1) dR.l=r.l||0;
      if(_fList.indexOf('b')>-1) dR.b=r.b||0;
      if(_fList.indexOf('d')>-1) dR.d=r.d||0;
      if(_fList.length) dR.isDeduct=(r.isDeduct||false); /* mirror deduction flag only if something is linked */
      var _nt=(dR.n||1);
      ...
      if(_dRow){
        ...
        /* inputs order in row: lbl, n, l, b, d — find by oninput attr; only update fields that are actually linked */
        _dRow.querySelectorAll('input').forEach(function(inp){
          var _oi=inp.getAttribute('oninput')||'';
          if(_fList.indexOf('n')>-1&&_oi.indexOf(",'n')")>-1) inp.value=dR.n||1;
          if(_fList.indexOf('l')>-1&&_oi.indexOf(",'l')")>-1) inp.value=dR.l||'';
          if(_fList.indexOf('b')>-1&&_oi.indexOf(",'b')")>-1) inp.value=dR.b||'';
          if(_fList.indexOf('d')>-1&&_oi.indexOf(",'d')")>-1) inp.value=dR.d||'';
        });
        /* Update qty display */
...
window._msrLinkFields={n:true,l:true,b:true,d:true};
...
function msrPasteRows(destIIdx){
  ...
  var _lf=window._msrLinkFields||{n:false,l:true,b:true,d:false};
  var _linkedFieldList=['n','l','b','d'].filter(function(f){return _lf[f];});
  if(!_linkedFieldList.length){alert('Kripaya kiman ek field (N/L/B/D) link sathi nivda.');return;}
  ...
  selList.forEach(function(src){
    ...
    var newRow={
      ...
      _linkedFrom: {iIdx: src.iIdx, rIdx: src.rIdx, fields: _linkedFieldList.slice()}  /* index ref — survives JSON save/load */
    };
    var _t=(newRow.n||1);
  ...
  for(i=0;i<items.length;i++){
    ...
    for(ri=0;ri<it.rows.length;ri++){
      ...
      if(!r.isTot){
        ...
        if(r._linkedFrom){
          ...
          if(_s){
            ...
            if(_flist.indexOf('l')>-1) r.l=(_s.l!=null?_s.l:0);
            if(_flist.indexOf('b')>-1) r.b=(_s.b!=null?_s.b:0);
            if(_flist.indexOf('d')>-1) r.d=(_s.d!=null?_s.d:0);
            if(_flist.indexOf('n')>-1) r.n=(_s.n||1);
      ...
      if(isTotRow){
        ...
        var _nLinked=_lnkFields.indexOf('n')>-1;
        var _lLinked=_lnkFields.indexOf('l')>-1;
        var _bLinked=_lnkFields.indexOf('b')>-1;
        var _dLinked=_lnkFields.indexOf('d')>-1;
        var _lnkTitle='';
        if(_isLinked){
          ...
          var _fNames={n:'N',l:'L',b:'B',d:'D'};
          var _fTxt=_lnkFields.map(function(f){return _fNames[f];}).join('+')||'—';
          _lnkTitle='🔗 '+_fTxt+' linked from Item '+(_lf2.iIdx+1)+', Row '+(_lf2.rIdx+1)+' ('+_srcDescLnk2+') — edit source row to update';
        }
    ...
    h+='<tr class="noprt"><td colspan="11" style="padding:.25rem .4rem;border-top:1px solid #e8e8e8">'
      +(window._msrSelActive?('<span style="font-size:.56rem;color:var(--mu);margin-right:.3rem">Link:</span>'
        +['n','l','b','d'].map(function(f){
          var _lbl={n:'N',l:'L',b:'B',d:'D'}[f];
