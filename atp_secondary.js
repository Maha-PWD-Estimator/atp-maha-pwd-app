...
function aShowSec(sec){["pay","users","ests","settings","loginpage","ssr","lead","mtf","diary"].forEach(function(s){var e=G("asec-"+s);if(e)e.style.display=s===sec?"block":"none";});document.querySelectorAll(".asectab").forEach(function(t,i){t.classList.toggle("on",["pay","users","ests","settings","loginpage","ssr","lead","mtf","diary"][i]===sec);});if(sec==="settings"){aLoadCurQR();aLoadSettings();if(typeof aLoadBitRefImage==="function")aLoadBitRefImage();}if(sec==="loginpage"){lpLoadAdmin();}if(sec==="ssr"){aRenderSSR();}if(sec==="lead"){aRenderLead();}if(sec==="mtf"&&typeof aLoadMTFCosts==="function"){aLoadMTFCosts();}if(sec==="diary"){aLoadDiary();}}
