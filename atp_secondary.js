...
function injectFeedbackWidget(){
  ...
  btn.innerHTML='💬';
  btn.style.cssText='position:fixed;bottom:18px;right:18px;width:52px;height:52px;border-radius:50%;background:#18602f;color:#fff;border:none;font-size:1.4rem;box-shadow:0 4px 14px rgba(0,0,0,.35);cursor:pointer;z-index:9005;display:flex;align-items:center;justify-content:center';
  var badge=document.createElement('span');
...
/* ════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════ */
var QRY_UNSUB_A=null,QRY_UNSUB_B=null,A_CHAT_UNSUB=null,A_CHAT_UID=null,A_CHAT_USERS_CACHE={};
...
function aInjectQueryInbox(){
  ...
  btn.innerHTML='💬';
  btn.style.cssText='position:fixed;bottom:18px;right:18px;width:52px;height:52px;border-radius:50%;background:#1565c0;color:#fff;border:none;font-size:1.4rem;box-shadow:0 4px 14px rgba(0,0,0,.35);cursor:pointer;z-index:9005;display:flex;align-items:center;justify-content:center';
  var badge=document.createElement('span');
  badge.id='aQryBadge';
  badge.style.cssText='display:none;position:absolute;top:-2px;right:-2px;background:#e53935;color:#fff;border-radius:8px;min-width:16px;height:16px;font-size:.6rem;font-weight:900;line-height:16px;text-align:center;padding:0 3px;box-sizing:border-box';
  btn.appendChild(badge);
...
function aStartQueryListener(){
  aStopQueryListener();
  if(!window.firebase||!firebase.firestore)return;
  var db=firebase.firestore(),counts={legacy:0,neu:0},prevTotal=-1;
  function recalc(){
  ...
  QRY_UNSUB_A=db.collection('queries').where('status','==','open').onSnapshot(function(s){counts.legacy=s.size;recalc();},function(e){console.error('Chat notif listener A error',e);QRY_UNSUB_A=null;});
  QRY_UNSUB_B=db.collection('queries').where('sender','==','user').where('seenByAdmin','==',false).onSnapshot(function(s){counts.neu=s.size;recalc();},function(e){console.error('Chat notif listener B error',e);showToast('Chat notifications error: '+e.message,'error');QRY_UNSUB_B=null;});
}
