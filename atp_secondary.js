  ...
  G("usrTbody").innerHTML=users.map(function(u){
    return "<tr>"+
      ...
      "<td style='display:flex;gap:.3rem'>"+
        ...
        "<button class='xbtn xbl' data-uid='"+u._id+"' data-nm='"+esc(u.name||u.email||"")+"' onclick='aOpenUserChat(this.dataset.uid,this.dataset.nm)'>&#128172;</button>"+
      "</td></tr>";
  }).join("");
}
...
function aStartQueryListener(){
  ...
  QRY_UNSUB_A=db.collection('queries').where('status','==','open').onSnapshot(function(s){counts.legacy=s.size;recalc();},function(){QRY_UNSUB_A=null;});
  QRY_UNSUB_B=db.collection('queries').where('sender','==','user').where('seenByAdmin','==',false).onSnapshot(function(s){counts.neu=s.size;recalc();},function(){QRY_UNSUB_B=null;});
}
...
function aLoadUserList(cb){
  ...
  firebase.firestore().collection('queries').orderBy('createdAt','desc').limit(500).get().then(function(snap){
    ...
    list.innerHTML=arr.map(function(u){
      return "<div class='aQryUserRow' data-uid='"+esc(u.uid)+"' data-nm='"+esc(u.name)+"' style='padding:.5rem .4rem;border-radius:8px;cursor:pointer;margin-bottom:.25rem;"+(u.uid===A_CHAT_UID?"background:#e3f2fd":"")+"'>"+
